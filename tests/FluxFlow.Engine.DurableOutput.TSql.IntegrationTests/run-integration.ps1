...
$resultsDirectory = Join-Path `
    ([IO.Path]::GetTempPath()) `
    ("FluxFlowTSqlIntegration_" + [Guid]::NewGuid().ToString("N"))
...
try
{
    ...
    else
    {
        ...
        $containerName = "fluxflow-tsql-tests-$runId"
    ...
    $project = Join-Path $PSScriptRoot "FluxFlow.Engine.DurableOutput.TSql.IntegrationTests.csproj"
    $testArguments = @(
        "test",
        $project,
