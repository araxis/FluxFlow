...
public sealed class ComponentFamilyRegistrationMatrixTests
{
    ...
    [Fact]
    public void Explicit_family_matrix_exposes_only_flow_value_normal_data_ports()
    {
        var normalDataPorts = new List<(string Family, string Component, ComponentPortMetadata Port)>();
        ...
        foreach (var family in Families)
        {
            ...
            {
                normalDataPorts.AddRange(descriptor.Inputs.Values
                    .Where(static port => port.Kind == ComponentPortKind.Message)
                    .Select(port => (family.Name, descriptor.Type, port)));
                normalDataPorts.AddRange(descriptor.Outputs.Values
                    .Where(static port =>
                        port.Kind == ComponentPortKind.Message &&
                        port.MessageType != typeof(global::FluxFlow.Data.FlowValue))
                    .Select(port => (family.Name, descriptor.Type, port)));
            }
        }

        normalDataPorts.ShouldNotBeEmpty();
        foreach (var (family, component, port) in normalDataPorts)
        {
            port.MessageType.ShouldBe(
                typeof(FlowValue),
                $"{family} '{component}.{port.Name}' must expose FlowValue at the runtime-authored workflow boundary.");
        }
    }

    [Fact]
    public void Explicit_family_matrix_publishes_shapes_for_every_flow_value_input()
    {
        var shapedInputs = new List<(string Family, string Component, PortDesignMetadata Port)>();
        ...
        foreach (var family in Families)
        {
            ...
            foreach (var component in designCatalog.All)
            {
                foreach (var port in component.Ports.Where(static port =>
                             port.Direction == PortDirection.Input &&
                             port.Kind == ComponentPortKind.Message &&
                             port.MessageType == typeof(FlowValue)))
                {
                    port.InputShape.ShouldNotBeNull(
                        $"{family.Name} '{component.Type.Value}.{port.Name.Value}' must publish its accepted FlowValue shape.");
                    port.InputShape.Description.ShouldNotBeNullOrWhiteSpace();
                    shapedInputs.Add((family.Name, component.Type.Value, port));
                }

                component.Ports
                    .Where(static port => port.Direction == PortDirection.Output)
                    .ShouldAllBe(static port => port.InputShape == null);
            }
        }

        shapedInputs.ShouldNotBeEmpty();
    }
    ...
    public void Explicit_family_matrix_keeps_all_44_explicit_event_outputs()
    {
        ...
        foreach (var family in Families)
        {
            ...
            foreach (var descriptor in ReadDescriptors(services))
            {
                ...
                events.ShouldNotBeNull().MessageType.ShouldBe(typeof(global::FluxFlow.Data.FlowValue));
                ...
                designedEvents.MessageType.ShouldBe(typeof(global::FluxFlow.Data.FlowValue));
                designedEvents.ValueType?.Value.ShouldBe(nameof(global::FluxFlow.Data.FlowValue));
    ...
    [Theory]
    [InlineData(@"..\..\src\Components\Designer\FluxFlow.Components.Designer\FluxFlow.Components.Designer.csproj")]
    [InlineData("../../src/Components/Designer/FluxFlow.Components.Designer/FluxFlow.Components.Designer.csproj")]
