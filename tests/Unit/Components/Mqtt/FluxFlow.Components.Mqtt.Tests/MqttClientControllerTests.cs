...
public sealed class MqttClientControllerTests
{
    ...
    public async Task ControlNodePreservesInputOrderWhilePublishingConcurrently()
    {
        ...
        await node.Input.SendAsync(MqttRequest(new MqttPublishClientRequest
        {
        ...
        var firstTopic = MqttResultTopic(
            await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5)));
        var secondTopic = MqttResultTopic(
            await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5)));
        firstTopic.ShouldBe("events/first");
        secondTopic.ShouldBe("events/second");
    ...
    public async Task ControlNodeFansOutEveryOrderedResultToTwoSubscribers()
    {
        ...
        firstMessages.Select(MqttResultTopic)
            .ShouldBe(["events/1", "events/2", "events/3"]);
        secondMessages.Select(MqttResultTopic)
            .ShouldBe(["events/1", "events/2", "events/3"]);
    ...
    public async Task ControlNodeCanEmitConcurrentResultsInCompletionOrder()
    {
        ...
        var firstResultTopic = MqttResultTopic(
            await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5)));
        firstResultTopic.ShouldBe("events/second");
    }
    ...
    public async Task MissingNamedSubscriptionWaitsUntilControlAddsIt()
    {
        ...
        var receivedMessage = received.Value!.Deserialize<MqttReceivedApplicationMessage>()!;
        receivedMessage.Topic.ShouldBe("commands/two");
        receivedMessage.MatchedSubscriptions.ShouldBe(["commands"]);
    }
    ...
    public async Task AckSignalUsesTraceIdentityAndCompletesBrokerOutcomeOnce()
    {
        ...
        await WaitUntilAsync(() => events.TryReceiveAll(out var values) &&
            values.Any(@event => @event.ToFlowEvent().Name == "mqtt.receive.outcome-ignored"));
    }
    ...
    [Fact]
    public async Task ControlNodeRejectsMalformedWorkflowValueInsideVisibleComponent()
    {
        var session = new VNextRecordingMqttTransportSession();
        await using var controller = CreateController(session);
        await controller.StartAsync();
        await using var node = new MqttControlNode(controller);
        var output = MqttTestContext.Sink(node.Output);
        var events = MqttTestContext.Sink(node.Events);
        var malformed = FlowMessage.Create(FlowValue.From(42));

        (await node.Input.SendAsync(malformed)).ShouldBeTrue();

        var failure = await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5));
        failure.IsError.ShouldBeTrue();
        failure.Error!.Code.ShouldBe("mqtt.command.invalid_shape");
        var @event = await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5));
        @event.ToFlowEvent().Name.ShouldBe("mqtt.command.rejected");
        @event.ToFlowEvent().Attributes["errorCode"].ShouldBe("mqtt.command.invalid_shape");
        session.Published.ShouldBeEmpty();
    }

    [Fact]
    public async Task PublishNodeRejectsMalformedWorkflowValueInsideVisibleComponent()
    {
        var session = new VNextRecordingMqttTransportSession();
        await using var controller = CreateController(session);
        await controller.StartAsync();
        ...
        var failure = await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5));
        failure.IsError.ShouldBeTrue();
        failure.Error!.Code.ShouldBe("mqtt.publish.invalid_shape");
        var @event = await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5));
        @event.ToFlowEvent().Name.ShouldBe("mqtt.publish.rejected");
        @event.ToFlowEvent().Attributes["errorCode"].ShouldBe("mqtt.publish.invalid_shape");
        session.Published.ShouldBeEmpty();
    }
    ...
    public async Task PublishNodePreservesLineageAndContinuesAfterInvalidInput()
    {
        ...
        var invalid = FlowMessage.Create(
            FlowValue.From(new MqttPublishMessage
            {
                Topic = "invalid/#",
    ...
    public async Task EventsNodeEmitsConnectionDomainEvents()
    {
        ...
        var connected = @event.Value!.Deserialize<MqttClientConnectedEvent>()!;
        connected.Client.ShouldBe("client-1");
    }
    ...
    private static FlowMessage MqttRequest(MqttClientRequest request)
        => FlowMessage.Create(FlowValue.From(
            JsonSerializer.SerializeToElement<MqttClientRequest>(
                request,
                new JsonSerializerOptions(JsonSerializerDefaults.Web))));

    private static string MqttResultTopic(FlowMessage message)
    {
        message.IsError.ShouldBeFalse();
        return message.Value.ToJsonElement().GetProperty("topic").GetString()!;
    }
