...
public sealed class RequestReplyCoordinatorTests
{
    ...
    public async Task RequestReplyMode_EmitsPublishedEventAfterRequestReachesOutput()
    {
        ...
        received.ToFlowEvent().Name.ShouldBe(RequestReplyEvents.Received);
        received.CorrelationId.ShouldBe(request.CorrelationId);
        published.ToFlowEvent().Name.ShouldBe(RequestReplyEvents.Published);
        published.CorrelationId.ShouldBe(request.CorrelationId);
        replied.ToFlowEvent().Name.ShouldBe(RequestReplyEvents.Replied);
    ...
    public async Task UnmatchedResponse_ReportsError()
    {
        ...
        error.ToFlowEvent().Attributes["code"].ShouldBe(RequestReplyErrorCodes.Unmatched);
    ...
    public async Task NullRequestContext_ReportsError_AndContinues()
    {
        ...
        error.ToFlowEvent().Attributes["code"].ShouldBe(RequestReplyErrorCodes.InvalidRequestContext);
        error.CorrelationId.ShouldBeNull();
        flowEvent.ToFlowEvent().Name.ShouldBe(RequestReplyEvents.Invalid);
    ...
    public async Task NullResponseMessage_ReportsError_AndContinues()
    {
        ...
        var error = await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
        var flowEvent = await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
        error.ToFlowEvent().Attributes["code"].ShouldBe(RequestReplyErrorCodes.InvalidResponseMessage);
        error.CorrelationId.ShouldBeNull();
        flowEvent.ToFlowEvent().Name.ShouldBe(RequestReplyEvents.Invalid);
        flowEvent.ToFlowEvent().Level.ShouldBe(FlowEventLevel.Error);
    ...
    private static async Task<FlowEvent> ReceiveErrorEventAsync(BufferBlock<FlowMessage> events)
    {
        while (true)
        {
            var @event = await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
            if (@event.ToFlowEvent().Attributes.ContainsKey("code"))
            {
                return @event.ToFlowEvent();
            }
        }
    }
