...
public sealed class MetricsServiceCollectionExtensionsTests
{
    ...
    public void AddMetrics_registers_request_result_metadata()
    {
        ...
        registration.Inputs[MetricsComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        registration.Outputs[MetricsComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
    }
    ...
    public async Task Hosted_metrics_aggregate_exposes_events()
    {
        await WithNodeAsync(async (ports, _) =>
        {
            ...
            @event.ToFlowEvent().Name.ShouldBe(MetricsDiagnosticNames.AggregateUpdated);
            eventMessage.CorrelationId.ShouldBe(message.CorrelationId);
                @event.ToFlowEvent().Attributes["sampleCount"].ShouldBe(1d);
        });
    }
