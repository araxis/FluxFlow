...
public sealed class HttpClientNodeTests
{
    ...
    [Fact]
    public async Task Malformed_workflow_value_is_rejected_inside_the_visible_node_and_processing_continues()
    {
        ...
        var malformed = FlowMessage.Create(
            FlowValue.From(42),
            new CorrelationId("http-malformed"));

        (await node.Input.SendAsync(malformed)).ShouldBeTrue();
        (await node.Input.SendAsync(HttpRequest(new HttpClientRequest
        {
            Url = "https://example.test/valid"
        }))).ShouldBeTrue();

        var rejected = await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
        rejected.IsError.ShouldBeTrue();
        rejected.Error!.Code.ShouldBe("http.request.invalid_shape");
        rejected.CorrelationId.ShouldBe(malformed.CorrelationId);
        var @event = await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
        @event.ToFlowEvent().Name.ShouldBe(HttpClientNode.RequestRejected);
        @event.CorrelationId.ShouldBe(malformed.CorrelationId);
        @event.ToFlowEvent().Attributes["errorCode"].ShouldBe("http.request.invalid_shape");

        HttpResponse(await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30)))
    ...
    public async Task Request_and_response_preserve_exact_content_and_message_lineage()
    {
        ...
        var result = message.Value!.Deserialize<HttpResponseResult>()!;
    ...
    public async Task Output_fans_out_every_result_to_every_consumer()
    {
        ...
        DecodeText((await first.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30)))
            .Value!.Deserialize<HttpResponseResult>()!.Body).ShouldBe("r1");
        DecodeText((await first.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30)))
            .Value!.Deserialize<HttpResponseResult>()!.Body).ShouldBe("r2");
    ...
    public async Task Relative_url_resolves_against_client_base_address()
    {
        ...
        (await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30)))
            .Value!.Deserialize<HttpResponseResult>()!.StatusCode.ShouldBe(200);
    ...
    public async Task Configured_non_success_response_is_an_error_result_and_later_input_continues()
    {
        ...
        var success = (await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30)))
            .Value!.Deserialize<HttpResponseResult>()!;
    ...
    public async Task Incoming_error_is_propagated_without_sending_and_later_input_continues()
    {
        ...
        var upstreamError = new FlowError(
            ...
            "test");
        await node.Input.SendAsync(FlowMessage.CreateError(upstreamError));
        await node.Input.SendAsync(HttpRequest(new HttpClientRequest
        {
    ...
    public async Task Network_failure_is_a_normal_error_result()
    {
        ...
        var output = Sink(node.Output);
        var events = Sink(node.Events);
        var request = HttpRequest(new HttpClientRequest
        {
            Url = "https://example.test/status"
        });
        ...
        var @event = await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
        @event.ToFlowEvent().Name.ShouldBe(HttpClientNode.RequestFailed);
    ...
    public async Task Response_body_preserves_and_honors_declared_charset(string charset)
    {
        ...
        var body = (await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30)))
            .Value!.Deserialize<HttpResponseResult>()!.Body;
    ...
    public async Task Success_emits_a_correlated_completion_event()
    {
        ...
        var @event = await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
        @event.ToFlowEvent().Name.ShouldBe(HttpClientNode.RequestCompleted);
    ...
    private static FlowMessage HttpRequest(
    ...
    private static HttpResponseResult HttpResponse(FlowMessage message)
    {
        message.IsError.ShouldBeFalse();
        return message.Value.Deserialize<HttpResponseResult>()!;
    }
