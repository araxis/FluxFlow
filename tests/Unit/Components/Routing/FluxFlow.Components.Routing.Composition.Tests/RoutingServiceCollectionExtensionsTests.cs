...
public sealed class RoutingServiceCollectionExtensionsTests
{
    ...
    public void Design_metadata_provider_describes_static_routing_ports()
    {
        ...
        AssertPorts(
            ...
            [
                (RoutingComponentDefinition.Ports.Input, PortDirection.Input, 0, true, nameof(FlowValue)),
                (RoutingComponentDefinition.Ports.Output, PortDirection.Output, 1, true, nameof(FlowValue)),
                (RoutingComponentDefinition.Ports.Events, PortDirection.Output, 2, false, nameof(global::FluxFlow.Data.FlowValue))
            ]);
        AssertPorts(
        ...
        AssertPorts(
            ...
            [
                (RoutingComponentDefinition.Ports.Left, PortDirection.Input, 0, true, nameof(FlowValue)),
                (RoutingComponentDefinition.Ports.Right, PortDirection.Input, 1, false, nameof(FlowValue)),
                (RoutingComponentDefinition.Ports.Output, PortDirection.Output, 2, true, nameof(FlowValue)),
                (RoutingComponentDefinition.Ports.Events, PortDirection.Output, 3, false, nameof(global::FluxFlow.Data.FlowValue))
            ]);
    }
    ...
    public async Task Hosted_canonical_correlation_resolves_json_selectors()
    {
        ...
        var outputResult = ports.ReceiveAsync<FlowValue>(
            Port(RoutingComponentDefinition.Ports.Output),
            Timeout);
        ...
        result.Value.ToJsonElement()
            .GetProperty("match")
            .GetProperty("key")
            .GetString().ShouldBe("A-350");
    }
    ...
    public async Task Hosted_join_resolves_selectors_and_routes_matches()
    {
        ...
        var match = result.Value.ToJsonElement().GetProperty("match");
        match.GetProperty("key").GetString().ShouldBe("A-400");
        match.GetProperty("left").GetProperty("value").GetString().ShouldBe("left");
        match.GetProperty("right").GetProperty("value").GetString().ShouldBe("right");
        match.GetProperty("joinedAt").GetDateTimeOffset().ShouldBe(timestamp);
    }
