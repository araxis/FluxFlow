...
public sealed class FlowContentFileSystemNodeTests
{
    ...
    public async Task Read_preserves_exact_content_and_message_lineage()
    {
        ...
        var result = message.Value!.Deserialize<FileReadContent>()!;
    ...
    public async Task Text_read_records_normalized_encoding_without_decoding_bytes()
    {
        ...
        await node.Input.SendAsync(FileRequest(new FileReadRequest
        {
            Path = "input.txt",
        ...
        var content = (await output.ReceiveAsync().WaitAsync(TestTimeout))
            .Value!.Deserialize<FileReadContent>()!.Content;
    ...
    public async Task Missing_read_is_normal_failure_and_later_input_continues()
    {
        ...
        await node.Input.SendAsync(FileRequest(new FileReadRequest { Path = "missing.txt" }));
        await node.Input.SendAsync(FileRequest(new FileReadRequest { Path = "valid.txt" }));
        ...
        success.Value.Deserialize<FileReadContent>()!.Content.Bytes.AsSpan().ToArray()
    ...
    public async Task Read_success_emits_correlated_event_and_completes_cleanly()
    {
        ...
        value.ToFlowEvent().Name.ShouldBe(FileReadNode.ReadSucceeded);
        value.CorrelationId.ShouldBe(input.CorrelationId);
        value.ToFlowEvent().Attributes["bytesRead"].ShouldBe(5L);
    ...
    public async Task Write_uses_exact_original_bytes_and_preserves_lineage()
    {
        ...
        message.CausationId.ShouldBe(input.MessageId);
        message.IsError.ShouldBeFalse();
        var result = message.Value.Deserialize<FileWriteResult>()!;
        result.WrittenAt.ShouldBe(timestamp);
        result.BytesWritten.ShouldBe(bytes.Length);
    ...
    public async Task Incoming_error_propagates_and_later_write_continues()
    {
        ...
        await node.Input.SendAsync(FlowMessage.CreateError(
            new FlowError("upstream.failed", "Upstream failed.", "test")));
        await node.Input.SendAsync(FileRequest(new FileContentWriteRequest
        {
            Path = "valid.txt",
    ...
    public async Task Write_success_emits_correlated_event_and_completes_cleanly()
    {
        ...
        await output.ReceiveAsync().WaitAsync(TestTimeout);
        var value = await events.ReceiveAsync().WaitAsync(TestTimeout);
        value.ToFlowEvent().Name.ShouldBe(FileWriteNode.WriteSucceeded);
        value.CorrelationId.ShouldBe(input.CorrelationId);
        value.ToFlowEvent().Attributes["bytesWritten"].ShouldBe(5L);
    ...
    public async Task Directory_source_emits_typed_entries_and_faults_on_source_failure()
    {
                         ...
                         }))
        {
            ...
            var value = (await output.ReceiveAsync().WaitAsync(TestTimeout))
                .Value!.Deserialize<DirectoryEntry>()!;
    ...
    [Fact]
    public async Task Read_rejects_malformed_value_without_faulting_and_accepts_later_input()
    {
        using var directory = TempDirectory.Create("canonical-read-rejected");
        await File.WriteAllTextAsync(Path.Combine(directory.Path, "valid.txt"), "ok");
        await using var node = new FileReadNode(
            new FileReadOptions { BaseDirectory = directory.Path });
        var output = Sink(node.Output);
        ...
        var rejection = await output.ReceiveAsync().WaitAsync(TestTimeout);
        rejection.IsError.ShouldBeTrue();
        rejection.Error.ShouldNotBeNull();
        (await events.ReceiveAsync().WaitAsync(TestTimeout)).ToFlowEvent().Name.ShouldBe("file.read.rejected");
    ...
    private static FlowMessage FileRequest<T>(
        T request,
