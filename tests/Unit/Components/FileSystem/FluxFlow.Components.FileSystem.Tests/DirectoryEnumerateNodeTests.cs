...
public sealed class DirectoryEnumerateNodeTests
{
    ...
    public async Task DirectoryEnumerate_EmitsMatchingFilesInsideBaseDirectory()
    {
        ...
        var entries = (await DrainAsync(output))
            .Select(message => message.Value!.Deserialize<DirectoryEntry>()!)
            .ToList();
    ...
    public async Task DirectoryEnumerate_UsesInjectedClockForEntryTimestamp()
    {
        ...
        entry.Value!.Deserialize<DirectoryEntry>()!.EnumeratedAt.ShouldBe(enumeratedAt);
    }
    ...
    private static async Task<List<FlowEvent>> DrainEventsAsync(BufferBlock<FlowMessage> events)
    {
        ...
        while (await events.OutputAvailableAsync(cancellation.Token))
        {
            ...
            {
                collected.Add(value.ToFlowEvent());
            }
        }
