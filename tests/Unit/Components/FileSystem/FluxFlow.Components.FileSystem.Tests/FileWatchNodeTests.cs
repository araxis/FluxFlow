...
public sealed class FileWatchNodeTests
{
    ...
    private static async Task<FlowMessage<FileChange>> ReceiveMatchingAsync(
        BufferBlock<FlowMessage> output,
        Func<FileChange, bool> predicate)
    {
        ...
        while (!cancellation.IsCancellationRequested)
        {
            ...
            var change = value.Value!.Deserialize<FileChange>()!;
            if (predicate(change))
            {
                return value.ConvertValue(change);
            }
        }
    ...
    private static async Task<FlowEvent> ReceiveEventAsync(BufferBlock<FlowMessage> events, string name)
    {
        ...
        while (!cancellation.IsCancellationRequested)
        {
            ...
            if (value.ToFlowEvent().Name == name)
            {
                return value.ToFlowEvent();
            }
        }
