...
public sealed class FileSystemServiceCollectionExtensionsTests
{
    ...
    public void AddFileSystem_registers_request_result_metadata()
    {
        ...
        read.Inputs[FileSystemComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        read.Outputs[FileSystemComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));

        var write = registry.Components[FileSystemComponentDefinition.Types.Write];
        write.Inputs[FileSystemComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        write.Outputs[FileSystemComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));

        registry.Components[FileSystemComponentDefinition.Types.DirectoryEnumerate]
            .Outputs[FileSystemComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
        ...
        await WithTransformNodeAsync<FileReadRequest, FileReadContent>(
            ...
            async (ports, host) =>
            {
                ...
                var resultReceive = ports.ReceiveAsync<FileReadContent>(
                    Output,
                    Timeout);
                var eventReceive = ports.ReceiveAsync<global::FluxFlow.Data.FlowValue>(
                    Events,
                    Timeout);
                ...
                @event.ToFlowEvent().Name.ShouldBe(FileSystemDiagnosticNames.FileReadSucceeded);
                @event.ToFlowEvent().Timestamp.ShouldBe(timestamp);
    ...
    public async Task Hosted_file_write_writes_under_base_directory_and_preserves_correlation_id()
    {
        ...
        await WithTransformNodeAsync<FileContentWriteRequest, FileWriteResult>(
            ...
            async (ports, host) =>
            {
                ...
                var @event = (await eventReceive).Message.ShouldNotBeNull();
                @event.CorrelationId.ShouldBe(message.CorrelationId);
                @event.ToFlowEvent().Name.ShouldBe(FileSystemDiagnosticNames.FileWriteSucceeded);
                @event.ToFlowEvent().Timestamp.ShouldBe(timestamp);
                await host.RevisionHost.StopAsync();
            },
    ...
    public async Task Hosted_directory_enumerate_starts_through_runtime_and_completes()
    {
        ...
        var events = new MessageTracker<global::FluxFlow.Data.FlowValue>();
        ...
        var completed = await events.WaitForAsync(value =>
            value.ToFlowEvent().Name == FileSystemDiagnosticNames.DirectoryEnumerateCompleted);
        completed.ToFlowEvent().Timestamp.ShouldBe(timestamp);
    ...
    public async Task Hosted_file_watch_starts_observes_change_and_stops()
    {
        ...
        var started = await events.WaitForAsync(value =>
            value.ToFlowEvent().Name == FileSystemDiagnosticNames.FileWatchStarted);
        started.ToFlowEvent().Timestamp.ShouldBe(timestamp);
        ...
        await events.WaitForAsync(value =>
            value.ToFlowEvent().Name == FileSystemDiagnosticNames.FileWatchChanged);
        await host.RevisionHost.StopAsync();
    }
    ...
    private static async ValueTask<CanonicalApplicationTestHost> StartSourceNodeAsync<T>(
        ...
        MessageTracker<global::FluxFlow.Data.FlowValue> events,
        Action<IServiceCollection> configureComponents)
    {
        ...
        return await CanonicalApplicationTestHost.StartAsync(
            ...
            services =>
            {
                ...
                RegisterFlowValueRecorder(services, ValueRecorderType, values);
                RegisterFlowValueRecorder(services, EventRecorderType, events);
            },
    ...
    private static void RegisterFlowValueRecorder<T>(
        IServiceCollection services,
        string nodeType,
        MessageTracker<T> tracker)
        => services.AddFluxFlowComponents().Advanced.AddDynamicComponent(nodeType, component =>
            component
                .UseFactory(_ => new FlowValueRecordingNode<T>(tracker))
                .HasFlowValueInput("Input", static node => node.Input));
        ...
        ComponentDesignMetadata metadata)
    {
        ...
        output.ValueType?.Value.ShouldBe(nameof(FlowValue));
    ...
    private sealed class FlowValueRecordingNode<T>(MessageTracker<T> tracker)
        : FlowNode
    {
        private readonly TestFlowValueMaterializer<T> _materializer = new();

        protected override Task ProcessAsync(FlowMessage message)
        {
            if (message.IsError)
            {
                tracker.Add(message.ConvertError<T>());
                return Task.CompletedTask;
            }

            var materialization = _materializer.Materialize(message.Value);
            tracker.Add(materialization.IsSuccess
                ? message.ConvertValue(materialization.Value)
                : message.WithError(materialization.Error!).ConvertError<T>());
            return Task.CompletedTask;
        }
    }

    private sealed class TestFlowValueMaterializer<T>()
        : JsonFlowValueMaterializer<T>(
            "test.recording.invalid",
            "Test recorder",
            FlowValueShape.Any("Test recorder input."));
