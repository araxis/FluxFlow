...
public sealed class ExpectationsServiceCollectionExtensionsTests
{
    ...
    public void AddExpectations_registers_only_the_canonical_contract()
    {
        ...
        registration.Inputs[ExpectationsComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        registration.Outputs[ExpectationsComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
    }
    ...
    public async Task Canonical_host_exposes_correlated_events()
    {
        await WithNodeAsync(
            async (ports, _) =>
            {
                ...
                @event.ToFlowEvent().Name.ShouldBe(ExpectationDiagnosticNames.Matched);
                @event.ToFlowEvent().Attributes["satisfied"].ShouldBe(true);
