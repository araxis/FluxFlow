...
public sealed class StateServiceCollectionExtensionsTests
{
    ...
    public void AddState_registers_only_the_canonical_contract()
    {
        ...
        reducer.Inputs[StateComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        reducer.Outputs[StateComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
    }
    ...
    public async Task Canonical_host_updates_state_preserves_lineage_and_uses_clock()
    {
        ...
        await WithNodeAsync(
            ...
            async (ports, _) =>
            {
                ...
                @event.ToFlowEvent().Attributes["engine"].ShouldBe("sample");
                @event.ToFlowEvent().Attributes["expressionName"].ShouldBe("counter");
            },
            Properties(
                ("reducer", "count"),
                ("initialState", 10),
    ...
    public async Task Canonical_host_binds_key_expression_and_diagnostic_metadata()
    {
        await WithNodeAsync(
            ...
            async (ports, _) =>
            {
                ...
                var @event = (await eventReceive).Message.ShouldNotBeNull();
                @event.ToFlowEvent().Attributes["expressionId"].ShouldBe("state-1");
                @event.ToFlowEvent().Attributes["expressionName"].ShouldBe("last payload");
            },
            Properties(
                ("reducer", "last-input"),
