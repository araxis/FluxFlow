...
public sealed class StorageServiceCollectionExtensionsTests
{
    ...
    public void AddStorage_registers_canonical_metadata()
    {
        ...
        put.Inputs[StorageComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        put.Outputs[StorageComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));

        var get = registry.Components[StorageComponentDefinition.Types.Get];
        get.Inputs[StorageComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        get.Outputs.Keys.ShouldBe([
            StorageComponentDefinition.Ports.Output,
            "Events"
        ], ignoreOrder: false);
        get.Outputs[StorageComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));

        var query = registry.Components[StorageComponentDefinition.Types.Query];
        query.Inputs[StorageComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        query.Outputs.Keys.ShouldBe([
            StorageComponentDefinition.Ports.Output,
            "Events"
        ], ignoreOrder: false);
        query.Outputs[StorageComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));

        var delete = registry.Components[StorageComponentDefinition.Types.Delete];
        delete.Outputs[StorageComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
    }
    ...
    public async Task Hosted_put_resolves_store_binds_options_and_uses_clock()
    {
        ...
        await WithNodeAsync(
            ...
            async (ports, host) =>
            {
                ...
                @event.ToFlowEvent().Name.ShouldBe(StorageDiagnosticNames.PutStored);
        ...
        string outputType)
    {
        ...
        metadata.Ports[1].ValueType?.Value.ShouldBe(nameof(FlowValue));
        ...
        byte[] bytes)
    {
        ...
        await node.Input.SendAsync(FlowMessage.Create(
            FlowValue.From(new StorageContentPutRequest
            {
                Key = key,
