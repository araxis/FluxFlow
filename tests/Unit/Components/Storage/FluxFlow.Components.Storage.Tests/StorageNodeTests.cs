...
public sealed class StorageNodeTests
{
    ...
    public async Task Put_and_get_preserve_exact_content_metadata_and_lineage()
    {
        ...
        var stored = putMessage.Value.Deserialize<StoragePutOutcome>()!;
        ...
        var found = getMessage.Value.Deserialize<StorageGetOutcome>()!;
    ...
    public async Task Incoming_error_is_propagated_and_later_input_continues()
    {
        ...
        await node.Input.SendAsync(
            FlowMessage.CreateError(upstreamError));
        await node.Input.SendAsync(StorageRequest(new StorageContentPutRequest
        {
            Key = "valid",
        ...
        success.Value.Deserialize<StoragePutOutcome>()!.Key.ShouldBe("valid");
    ...
    public async Task Invalid_requests_are_normal_invalid_request_results()
    {
        ...
        await put.Input.SendAsync(StorageRequest(new StorageContentPutRequest
        {
            Key = " ",
            Content = FlowContent.FromBytes(new byte[] { 1 })
        }));
        await get.Input.SendAsync(StorageRequest(new StorageGetRequest { Key = " " }));
        await query.Input.SendAsync(StorageRequest(new StorageQueryRequest { Offset = -1 }));
        await delete.Input.SendAsync(StorageRequest(new StorageDeleteRequest { Key = " " }));
    ...
    public async Task Get_missing_and_legacy_content_are_normal_results()
    {
        ...
        await node.Input.SendAsync(StorageRequest(new StorageGetRequest { Key = "missing" }));
        await node.Input.SendAsync(StorageRequest(new StorageGetRequest { Key = "legacy" }));

        var missing = await output.ReceiveAsync().WaitAsync(Timeout);
        missing.IsError.ShouldBeFalse();
        missing.Value.Deserialize<StorageGetOutcome>()!.Found.ShouldBeFalse();
    ...
    public async Task Get_reads_existing_versioned_json_content()
    {
        ...
        await node.Input.SendAsync(StorageRequest(new StorageGetRequest { Key = "existing" }));

        var result = await output.ReceiveAsync().WaitAsync(Timeout);
        result.IsError.ShouldBeFalse();
        var outcome = result.Value.Deserialize<StorageGetOutcome>()!;
        outcome.Record.ShouldNotBeNull().Content.Bytes.AsSpan().ToArray()
            .ShouldBe(new byte[] { 0, 127, 255 });
        outcome.Record.Content.ContentType.ShouldBe("application/test");
        outcome.Record.Content.Encoding.ShouldBe("binary");
    }
    ...
    public async Task Query_returns_one_content_result_without_record_branching()
    {
        ...
        await using var query = new StorageQueryNode(
            ...
            new StorageQueryOptions { Collection = "items" });
        var queryOutput = StorageTestSink.Link(query.Output);
        await query.Input.SendAsync(StorageRequest(new StorageQueryRequest()));

        var result = (await queryOutput.ReceiveAsync().WaitAsync(Timeout))
            .Value!.Deserialize<StorageQueryOutcome>()!;
    ...
    public async Task Delete_always_returns_deleted_or_missing_outcome()
    {
        ...
        await delete.Input.SendAsync(StorageRequest(new StorageDeleteRequest { Key = "a" }));
        await delete.Input.SendAsync(StorageRequest(new StorageDeleteRequest { Key = "a" }));

        var deleted = await deleteOutput.ReceiveAsync().WaitAsync(Timeout);
        deleted.IsError.ShouldBeFalse();
        deleted.Value.Deserialize<StorageDeleteOutcome>()!.Deleted.ShouldBeTrue();
        var missing = await deleteOutput.ReceiveAsync().WaitAsync(Timeout);
        missing.IsError.ShouldBeFalse();
        missing.Value.Deserialize<StorageDeleteOutcome>()!.Found.ShouldBeFalse();
    }
    ...
    public async Task Store_failure_is_normal_and_later_input_continues()
    {
        ...
        await node.Input.SendAsync(StorageRequest(new StorageGetRequest { Key = "a" }));
        var failure = await output.ReceiveAsync().WaitAsync(Timeout);
        failure.IsError.ShouldBeTrue();
    ...
    public async Task Put_preserves_request_mode_override_and_record_suppression()
    {
        ...
        var created = (await output.ReceiveAsync().WaitAsync(Timeout))
            .Value!.Deserialize<StoragePutOutcome>()!;
        created.Version.ShouldBe(1);
        created.Record.ShouldBeNull();
        var replaced = (await output.ReceiveAsync().WaitAsync(Timeout))
            .Value!.Deserialize<StoragePutOutcome>()!;
    ...
    public async Task Store_identity_and_query_contract_violations_are_normal_failures()
    {
        ...
        await get.Input.SendAsync(StorageRequest(new StorageGetRequest { Key = "a" }));
        await query.Input.SendAsync(StorageRequest(new StorageQueryRequest
        {
            KeyPrefix = "user:",
            Limit = 1
    ...
    public async Task Output_fans_out_and_events_preserve_result_order()
    {
        ...
        (await StorageTestSink.DrainUntilCompletedAsync(events))
            .Select(item => item.ToFlowEvent().Name)
            .ShouldBe([
    ...
    [Fact]
    public async Task Put_rejects_malformed_value_without_faulting_and_accepts_later_input()
    {
        var store = new InMemoryStorageStore();
        await using var node = new StoragePutNode(
            store,
        ...
        var rejection = await output.ReceiveAsync().WaitAsync(Timeout);
        rejection.IsError.ShouldBeTrue();
        rejection.Error.ShouldNotBeNull();
        (await events.ReceiveAsync().WaitAsync(Timeout)).ToFlowEvent().Name.ShouldBe("storage.put.rejected");
    ...
    private static FlowMessage StorageRequest<T>(
        T request,
        CorrelationId? correlationId = null,
        TraceId? traceId = null)
        => FlowMessage.Create(FlowValue.From(request), correlationId, traceId);
