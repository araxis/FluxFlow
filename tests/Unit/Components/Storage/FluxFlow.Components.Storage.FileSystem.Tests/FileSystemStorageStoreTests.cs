...
public sealed class FileSystemStorageStoreTests
{
    ...
    public async Task Canonical_nodes_round_trip_exact_content_across_store_instances()
    {
                         ...
                         new StoragePutOptions { Collection = "items" }))
        {
            ...
            await put.Input.SendAsync(Message(new StorageContentPutRequest
            {
                Key = "content",
        ...
        await get.Input.SendAsync(Message(new StorageGetRequest { Key = "content" }));

        var record = (await results.ReceiveAsync()).Value!.Deserialize<StorageGetOutcome>()!.Record.ShouldNotBeNull();
