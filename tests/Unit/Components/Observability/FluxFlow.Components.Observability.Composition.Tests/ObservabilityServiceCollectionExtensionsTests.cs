...
public sealed class ObservabilityServiceCollectionExtensionsTests
{
    ...
    public async Task Hosted_logger_binds_options_and_resolves_selectors()
    {
        ...
        entry.Attributes["kind"].ShouldBeOfType<JsonElement>().GetString().ShouldBe("alpha");
    }
    ...
    public async Task Hosted_observability_component_exposes_events()
    {
        await WithNodeAsync(
            ...
            async ports =>
            {
                ...
                eventMessage.ToFlowEvent().Name.ShouldBe(ObservabilityDiagnosticNames.LoggerEmitted);
            });
    }
        ...
        string nodeType)
    {
        ...
        registration.Inputs[ObservabilityComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        registration.Outputs[ObservabilityComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
    }
