...
public sealed class ObservabilityNodeTests
{
    ...
    {
        var calls = 0;
        await using var node = new FlowCounterNode<JsonElement>(
    ...
    {
        await using var node = new FlowCounterNode<JsonElement>(new FlowCounterOptions());
    ...
    public async Task Counter_without_predicate_uses_clock_and_emits_correlated_event()
    {
        ...
        @event.ToFlowEvent().Name.ShouldBe(ObservabilityDiagnosticNames.CounterIncremented);
        @event.CorrelationId.ShouldBe(input.CorrelationId);
        @event.ToFlowEvent().Attributes["name"].ShouldBe("items");
        @event.ToFlowEvent().Attributes["nodeType"].ShouldBe("metric.count");
    }
    ...
    public async Task Logger_emits_typed_attributes_and_renders_template()
    {
        ...
        await using var node = new FlowLoggerNode<JsonElement>(
    ...
    {
        await using var node = new FlowLoggerNode<JsonElement>(new FlowLoggerOptions
        {
            Category = "workflow.test",
        ...
        var @event = await events.ReceiveAsync().WaitAsync(Timeout);
        @event.ToFlowEvent().Name.ShouldBe(ObservabilityDiagnosticNames.LoggerEmitted);
        @event.CorrelationId.ShouldBe(input.CorrelationId);
        @event.ToFlowEvent().Attributes["nodeType"].ShouldBe("log.write");
    }
    ...
    {
        var calls = 0;
        await using var node = new FlowMetricsNode<JsonElement>(
    ...
    public async Task Metrics_tracks_rate_and_averages_only_sized_observations()
    {
        ...
        firstEvent.ToFlowEvent().Name.ShouldBe(ObservabilityDiagnosticNames.MetricsObserved);
        secondEvent.CorrelationId.ShouldBe(secondInput.CorrelationId);
        secondEvent.ToFlowEvent().Attributes["nodeType"].ShouldBe("metric.measure");
    }

    [Fact]
    public void Constructors_validate_required_dependencies_and_options()
    {
        Should.Throw<ArgumentNullException>(() => new FlowCounterNode<JsonElement>(null!));
        Should.Throw<ArgumentNullException>(() => new FlowLoggerNode<JsonElement>(null!));
        Should.Throw<ArgumentNullException>(() => new FlowMetricsNode<JsonElement>(null!));
        Should.Throw<ArgumentNullException>(() => new FlowCounterNode<JsonElement>(
            new FlowCounterOptions { Predicate = "accepted" }));
        Should.Throw<ArgumentOutOfRangeException>(() => new FlowCounterNode<JsonElement>(
            new FlowCounterOptions { BoundedCapacity = 0 }));
        Should.Throw<ArgumentOutOfRangeException>(() => new FlowLoggerNode<JsonElement>(
            new FlowLoggerOptions { BoundedCapacity = 0 }));
        Should.Throw<ArgumentOutOfRangeException>(() => new FlowMetricsNode<JsonElement>(
            new FlowMetricsOptions { BoundedCapacity = 0 }));
        Should.Throw<InvalidOperationException>(() => new FlowLoggerNode<JsonElement>(
    ...
    {
        await using var node = new FlowMetricsNode<JsonElement>(new FlowMetricsOptions());
        var results = Sink(node.Output);
        var events = Sink(node.Events);
