...
public sealed class TimersServiceCollectionExtensionsTests
{
    ...
    public void Design_metadata_provider_describes_timer_ports()
    {
        ...
        AssertSourcePorts(metadata[TimersComponentDefinition.Types.Interval], nameof(FlowValue));
        AssertSourcePorts(metadata[TimersComponentDefinition.Types.Schedule], nameof(FlowValue));
        ...
        string outputType)
    {
        metadata.Ports.Select(port => (
            ...
            (TimersComponentDefinition.Ports.Events, PortDirection.Output, 2, false, nameof(global::FluxFlow.Data.FlowValue))
        ]);
    }
    ...
    private static void AssertTransformPorts(ComponentDesignMetadata metadata)
    {
        metadata.Ports.Select(port => (
            ...
            port.IsPrimary,
            port.ValueType?.Value)).ShouldBe([
            (TimersComponentDefinition.Ports.Input, PortDirection.Input, 0, true, nameof(FlowValue)),
            (TimersComponentDefinition.Ports.Output, PortDirection.Output, 1, true, nameof(FlowValue)),
            (TimersComponentDefinition.Ports.Events, PortDirection.Output, 2, false, nameof(global::FluxFlow.Data.FlowValue))
        ]);
    }
