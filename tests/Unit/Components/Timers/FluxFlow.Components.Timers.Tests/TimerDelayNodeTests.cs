...
public sealed class TimerDelayNodeTests
{
    ...
    public async Task Delay_emits_result_event()
    {
        ...
        flowEvent.ToFlowEvent().Name.ShouldBe(TimerDiagnosticNames.DelayEmitted);
        flowEvent.ToFlowEvent().Attributes["resultKind"].ShouldBe(TimerResultKinds.Delayed);
        flowEvent.ToFlowEvent().Attributes["nodeType"].ShouldBe("timer.delay");
    }
