...
public sealed class TimerThrottleNodeTests
{
    ...
    public async Task Throttle_emits_result_events()
    {
        ...
        flowEvent.ToFlowEvent().Name.ShouldBe(TimerDiagnosticNames.ThrottleEmitted);
        flowEvent.ToFlowEvent().Attributes["resultKind"].ShouldBe(TimerResultKinds.Throttled);
        flowEvent.ToFlowEvent().Attributes["nodeType"].ShouldBe("timer.throttle");
    }
