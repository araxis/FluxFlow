...
public sealed class ProjectionsServiceCollectionExtensionsTests
{
    ...
    public void AddProjections_registers_request_result_metadata()
    {
        ...
        registration.Inputs[ProjectionsComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        registration.Outputs[ProjectionsComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
    }
    ...
    public async Task Hosted_event_projection_exposes_events()
    {
        await WithNodeAsync(async (ports, _) =>
        {
            ...
            @event.ToFlowEvent().Name.ShouldBe(ProjectionDiagnosticNames.ProjectionUpdated);
            eventMessage.CorrelationId.ShouldBe(message.CorrelationId);
            @event.ToFlowEvent().Attributes["matchedCount"].ShouldBe(1d);
        });
    }
