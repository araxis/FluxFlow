...
public sealed class EventProjectionNodeTests
{
    ...
    public async Task Matching_events_emit_ordered_snapshots_with_message_lineage()
    {
        ...
        var ignored = ProjectionMessage(Event(timestamp.AddSeconds(-2), "ignored", status: "ok"));
        var second = ProjectionMessage(
        ...
        var firstSnapshot = firstResult.Value.Deserialize<EventProjectionSnapshot>()!;
        ...
        var secondSnapshot = secondResult.Value!.Deserialize<EventProjectionSnapshot>()!;
        ...
        diagnostic.ToFlowEvent().Name.ShouldBe(ProjectionDiagnosticNames.ProjectionUpdated);
        diagnostic.CorrelationId.ShouldBe(first.CorrelationId);
        diagnostic.ToFlowEvent().Attributes["resultKind"].ShouldBe(ProjectionResultKinds.Snapshot);
    }
    ...
    public async Task Projection_node_preserves_contract_owned_attributes_in_filter_and_snapshot_behavior()
    {
        ...
        await node.Input.SendAsync(ProjectionMessage(projectionEvent));

        var result = await results.ReceiveAsync().WaitAsync(Timeout);
        result.IsError.ShouldBeFalse();
        var snapshot = result.Value.Deserialize<EventProjectionSnapshot>()!;
    ...
    public async Task Incoming_error_is_propagated_and_later_input_continues()
    {
        ...
        var upstreamError = new FlowError(
            ...
            "Projections",
            isTransient: false);
        var missing = FlowMessage.CreateError(
            upstreamError,
            new CorrelationId("missing"));
        var valid = ProjectionMessage(
        ...
        success.Value!.Deserialize<EventProjectionSnapshot>()!.MatchedCount.ShouldBe(1);
    }
    ...
    public async Task Completion_emits_one_final_snapshot_after_accepted_input()
    {
        ...
        await using var node = new EventProjectionNode(
            ...
            clock);
        var results = Link(node.Output);
        var first = ProjectionMessage(Event(eventTime, "first"));
        var last = ProjectionMessage(
    ...
    public async Task Output_fans_out_each_result_to_every_consumer()
    {
        ...
        (await first.ReceiveAsync().WaitAsync(Timeout)).Value!.Deserialize<EventProjectionSnapshot>()!.MatchedCount.ShouldBe(1);
        (await second.ReceiveAsync().WaitAsync(Timeout)).Value!.Deserialize<EventProjectionSnapshot>()!.MatchedCount.ShouldBe(1);
    }
    ...
    public async Task Filter_applies_exclusions_identity_attributes_and_time_range()
    {
        ...
        await node.Input.SendAsync(ProjectionMessage(Event(
            from.AddSeconds(-1),
            "item.created",
            subject: "orders/1",
            channel: "events/live",
        ...
        var snapshot = (await results.ReceiveAsync().WaitAsync(Timeout))
            .Value!.Deserialize<EventProjectionSnapshot>()!;
    ...
    public async Task Null_filter_matches_every_event()
    {
        ...
        (await results.ReceiveAsync().WaitAsync(Timeout)).Value!.Deserialize<EventProjectionSnapshot>()!.MatchedCount.ShouldBe(1);
    }
    ...
    [Fact]
    public async Task Projection_rejects_malformed_value_without_faulting_and_accepts_later_input()
    {
        await using var node = new EventProjectionNode();
        ...
        await node.Input.SendAsync(EventMessage("valid"));
        ...
        (await events.ReceiveAsync().WaitAsync(Timeout)).ToFlowEvent().Name
            .ShouldBe("event.projection.rejected");
        var success = await output.ReceiveAsync().WaitAsync(Timeout);
        success.Value!.Deserialize<EventProjectionSnapshot>()!.MatchedCount.ShouldBe(1);
        node.Completion.IsFaulted.ShouldBeFalse();
    }

    private static FlowMessage EventMessage(string type)
        => ProjectionMessage(Event(DateTimeOffset.UtcNow, type));

    private static FlowMessage ProjectionMessage(
