...
public sealed class SessionComponentTests
{
    ...
    public async Task Recorder_passes_typed_content_to_the_store()
    {
        ...
        await node.Input.SendAsync(SessionRequest(ContentInput(7, "typed")));
        node.Complete();
    ...
    public async Task Recorder_preserves_content_lineage_order_and_existing_sequence()
    {
        ...
        await using var node = new SessionRecorderNode(
            ...
            store);
        var output = Sink(node.Output);
        var first = SessionRequest(
            ContentInput(1, "first"),
            headers: new Dictionary<string, string>
            {
                ["tenant"] = "north"
            });
        var second = SessionRequest(ContentInput(2, "second"));
        ...
        results.Select(result => result.Value!.Deserialize<SessionContentRecord>()!.Sequence)
            .ShouldBe([6L, 7L]);
        results.Select(result => result.Value!.Deserialize<SessionContentRecord>()!.Name)
            .ShouldBe(["first", "second"]);
        results[0].Value!.Deserialize<SessionContentRecord>()!.Content.Bytes.ToArray()
            .ShouldBe(new byte[] { 1 });
    ...
    public async Task Recorder_returns_start_failure_as_data_and_continues()
    {
        ...
        await using var node = new SessionRecorderNode(
            ...
            store);
        var output = Sink(node.Output);
        var failed = SessionRequest(ContentInput(1, "failed"));
        var succeeded = SessionRequest(ContentInput(2, "succeeded"));
        ...
        results[1].Value!.Deserialize<SessionContentRecord>()!.Sequence.ShouldBe(1);
    ...
    public async Task Recorder_propagates_input_and_append_failures_as_data_and_continues()
    {
        ...
        var upstreamError = new FlowError(
            ...
            "test");
        var unavailable = FlowMessage.CreateError(upstreamError);
        var appendFailure = SessionRequest(ContentInput(1, "append-failure"));
        var succeeded = SessionRequest(ContentInput(2, "succeeded"));
        ...
        results[2].Value!.Deserialize<SessionContentRecord>()!.Sequence.ShouldBe(1);
    ...
    public async Task Recorder_returns_null_append_result_as_data_and_continues()
    {
        ...
        await node.Input.SendAsync(SessionRequest(ContentInput(1, "failed")));
        await node.Input.SendAsync(SessionRequest(ContentInput(2, "succeeded")));
        node.Complete();
    ...
    public async Task Recorder_uses_clock_and_emits_lifecycle_events()
    {
        ...
        await node.Input.SendAsync(SessionRequest(ContentInput(1, "timed")));
        node.Complete();
        ...
        result.Value!.Deserialize<SessionContentRecord>()!.Timestamp.ShouldBe(timestamp);
        store.Metadata.ShouldNotBeNull().StartedAt.ShouldBe(timestamp);
        store.Metadata.EndedAt.ShouldBe(timestamp);
        diagnosticEvents.Select(@event => @event.ToFlowEvent().Name).ShouldContain(
    ...
    public async Task Recorder_reports_close_failure_without_faulting_normal_completion()
    {
        ...
        await node.Input.SendAsync(SessionRequest(ContentInput(1, "only")));
        node.Complete();
        ...
        diagnosticEvents.ShouldContain(@event =>
            @event.ToFlowEvent().Name == SessionsDiagnosticNames.RecorderFailed &&
            Equals(@event.ToFlowEvent().Attributes["errorCode"], SessionErrorCodeNames.CompleteFailed));
    }
    ...
    public async Task Replay_emits_exact_content_in_order_and_mints_message_identity()
    {
        ...
        results.Select(result => result.Value!.Deserialize<SessionContentRecord>()!.Sequence)
            .ShouldBe([2L, 3L]);
        results.Select(result => result.Value!.Deserialize<SessionContentRecord>()!.Content.Bytes[0])
    ...
    public async Task Replay_supports_fixed_interval()
    {
        ...
        (await DrainUntilCompletedAsync(output))
            .Select(result => result.Value!.Deserialize<SessionContentRecord>()!.Sequence)
            .ShouldBe([1L, 2L, 3L]);
    }
    ...
    public async Task Replay_cancellation_completes_without_late_output()
    {
        ...
        first.Value!.Deserialize<SessionContentRecord>()!.Sequence.ShouldBe(1);
        node.Completion.IsFaulted.ShouldBeFalse();
    ...
    public async Task Replay_returns_mid_stream_store_failure_as_normal_result()
    {
        ...
        results.Count.ShouldBe(2);
        results[0].Value!.Deserialize<SessionContentRecord>()!.Sequence.ShouldBe(1);
        results[1].Error.ShouldNotBeNull().Code.ShouldBe(
    ...
    public async Task Replay_returns_malformed_records_as_data_and_continues()
    {
        ...
        results[2].Value!.Deserialize<SessionContentRecord>()!.Sequence.ShouldBe(3);
    }
    ...
    public async Task Replay_returns_null_record_as_data_and_continues()
    {
        ...
        results[0].Error.ShouldNotBeNull().Code.ShouldBe(
            SessionErrorCodeNames.StoreUnavailable);
        results[1].Value!.Deserialize<SessionContentRecord>()!.Sequence.ShouldBe(2);
    }
    ...
    public async Task Query_returns_filtered_result_with_clock_and_lineage()
    {
        ...
        var output = Sink(node.Output);
        var events = Sink(node.Events);
        var input = SessionRequest(new SessionQueryRequest { CorrelationId = "corr-1" });
        ...
        result.Value.Deserialize<SessionQueryOutcome>()!.Count.ShouldBe(1);
        result.Value.Deserialize<SessionQueryOutcome>()!.Sessions.ShouldHaveSingleItem()
            .SessionId.ShouldBe("session-1");
        result.CorrelationId.ShouldBe(input.CorrelationId);
        result.CausationId.ShouldBe(input.MessageId);
        diagnostics.Single(@event => @event.ToFlowEvent().Name == SessionsDiagnosticNames.QueryCompleted)
    ...
    public async Task Query_can_omit_sessions_without_changing_count()
    {
        ...
        await node.Input.SendAsync(SessionRequest(new SessionQueryRequest()));
        node.Complete();
        var result = (await DrainUntilCompletedAsync(output)).ShouldHaveSingleItem();
        await node.Completion.WaitAsync(TimeSpan.FromSeconds(30));

        result.Value!.Deserialize<SessionQueryOutcome>()!.Count.ShouldBe(1);
        result.Value.Deserialize<SessionQueryOutcome>()!.Sessions.ShouldBeEmpty();
    }
    ...
    public async Task Query_returns_invalid_request_as_data_and_continues()
    {
        ...
        await using var node = new SessionQueryNode(
            ...
            store);
        var output = Sink(node.Output);
        var failed = SessionRequest(new SessionQueryRequest { Limit = 0 });
        var succeeded = SessionRequest(new SessionQueryRequest());
        ...
        results[0].CorrelationId.ShouldBe(failed.CorrelationId);
        results[1].Value!.Deserialize<SessionQueryOutcome>()!.Count.ShouldBe(1);
        results[1].CorrelationId.ShouldBe(succeeded.CorrelationId);
    }
    ...
    public async Task Query_rejects_store_results_outside_filter_as_data_and_continues()
    {
        ...
        await node.Input.SendAsync(SessionRequest(request));
        await node.Input.SendAsync(SessionRequest(request));
        node.Complete();
        var results = await DrainUntilCompletedAsync(output);
        await node.Completion.WaitAsync(TimeSpan.FromSeconds(30));
        ...
        results[1].Value!.Deserialize<SessionQueryOutcome>()!.Sessions.ShouldHaveSingleItem()
    ...
    public async Task Query_emits_started_event()
    {
        ...
        started.ToFlowEvent().Name.ShouldBe(SessionsDiagnosticNames.QueryStarted);
    }
    ...
    [Fact]
    public async Task Recorder_rejects_malformed_value_without_faulting_and_accepts_later_input()
    {
        var store = new TestSessionStore();
        await using var node = new SessionRecorderNode(
            new SessionRecorderOptions { SessionId = "session-1" },
        ...
        await node.Input.SendAsync(FlowMessage.Create(FlowValue.From(42)));
        await node.Input.SendAsync(SessionRequest(ContentInput(7, "valid")));

        var rejection = await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
        rejection.IsError.ShouldBeTrue();
        rejection.Error.ShouldNotBeNull();
        (await events.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30)))
            .ToFlowEvent().Name.ShouldBe("session.recorder.rejected");

        var success = await output.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(30));
        success.IsError.ShouldBeFalse();
        success.Value.Deserialize<SessionContentRecord>()!.Name.ShouldBe("valid");
        node.Completion.IsFaulted.ShouldBeFalse();
    }

    private static FlowMessage SessionRequest<T>(
        T request,
