...
public sealed class SessionsServiceCollectionExtensionsTests
{
    ...
    public void AddSessions_registers_canonical_metadata()
    {
        ...
        recorder.Inputs[SessionsComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        recorder.Outputs[SessionsComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));

        var replay = registry.Components[SessionsComponentDefinition.Types.Replay];
        replay.Inputs.ShouldBeEmpty();
        replay.Outputs[SessionsComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));

        var query = registry.Components[SessionsComponentDefinition.Types.Query];
        query.Inputs[SessionsComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        query.Outputs[SessionsComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
    ...
    public async Task Canonical_host_records_content_preserves_lineage_and_closes_session()
    {
            ...
            clock: clock))
        {
            ...
            var eventObservation = (await ports.ObserveAsync<global::FluxFlow.Data.FlowValue>(Events))
            ...
            new[] { firstEvent.ToFlowEvent().Name, secondEvent.ToFlowEvent().Name }.ShouldBe([
    ...
    public async Task Canonical_host_queries_with_options_clock_and_one_output()
    {
        ...
        var eventReceive = ports.ReceiveAsync<global::FluxFlow.Data.FlowValue>(Events, Timeout);
    ...
    private static void AssertQueryPorts(ComponentDesignMetadata metadata)
    {
        ...
        metadata.Ports[0].ValueType?.Value.ShouldBe(nameof(FlowValue));
        ...
        params (DateTimeOffset Timestamp, string Name, byte[] Bytes)[] records)
    {
        ...
        {
            await recorder.Input.SendAsync(FlowMessage.Create(
                FlowValue.From(new SessionContentRecordInput
                {
    ...
    {
        private readonly ActionBlock<FlowMessage> _input;
        ...
        public MessageRecordingNode(MessageTracker<T> tracker)
        {
            ...
            _input = new ActionBlock<FlowMessage>(message =>
            {
                if (message.IsError)
                {
                    tracker.Record(message.ConvertError<T>());
                    return;
                }

                var materialization = _materializer.Materialize(message.Value);
                tracker.Record(materialization.IsSuccess
