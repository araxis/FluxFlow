...
public sealed class PayloadInspectNodeTests
{
    ...
    public async Task Inspects_declared_json_and_preserves_content_and_message_identity()
    {
        ...
        var inspection = Read(received.Value);
        inspection.Timestamp.ShouldBe(timestamp);
        inspection.Content.ShouldNotBeNull().Bytes.ShouldBe(content.Bytes);
    ...
    public async Task Falls_back_to_utf8_for_an_invalid_declared_text_encoding()
    {
        ...
        await node.Input.SendAsync(Message(content));

        var inspection = Read((await output.ReceiveAsync().WaitAsync(Timeout)).Value!);
    ...
    public async Task Emits_invalid_declared_json_as_an_in_band_error_and_continues()
    {
        ...
        await node.Input.SendAsync(Message(invalid));
        await node.Input.SendAsync(Message(valid));
        ...
        Read(success.Value).Content.ShouldNotBeNull().Bytes.ShouldBe(valid.Bytes);
    ...
    public async Task Inspects_xml_and_text_base64_through_declared_media_types()
    {
        ...
        await node.Input.SendAsync(Message(xml));
        await node.Input.SendAsync(Message(base64));

        var xmlResult = Read((await output.ReceiveAsync().WaitAsync(Timeout)).Value!);
        xmlResult.Kind.ShouldBe(PayloadKind.Xml);
        xmlResult.FormattedPreview.ShouldNotBeNull().ShouldContain("<value>1</value>");

        var base64Result = Read((await output.ReceiveAsync().WaitAsync(Timeout)).Value!);
    ...
    public async Task Classifies_each_declared_json_shape(string json, PayloadKind expectedKind)
    {
        ...
        Read((await output.ReceiveAsync().WaitAsync(Timeout)).Value!).Kind.ShouldBe(expectedKind);
    }
    ...
    public async Task Preserves_empty_content_and_preview_limits()
    {
        ...
        var empty = Read((await output.ReceiveAsync().WaitAsync(Timeout)).Value!);
        empty.Kind.ShouldBe(PayloadKind.Empty);
        empty.TextPreview.ShouldBe(string.Empty);

        var truncated = Read((await output.ReceiveAsync().WaitAsync(Timeout)).Value!);
    ...
    private static PayloadInspectionResult Read(FlowValue value)
        => value.ToJsonElement().Deserialize<PayloadInspectionResult>(SerializerOptions)!;
}
