...
public sealed class PayloadsServiceCollectionExtensionsTests
{
    ...
    public void AddPayloads_registers_canonical_content_result_metadata()
    {
        ...
        registration.Inputs[PayloadsComponentDefinition.Ports.Input].MessageType.ShouldBe(typeof(FlowValue));
        registration.Outputs[PayloadsComponentDefinition.Ports.Output].MessageType.ShouldBe(typeof(FlowValue));
    }
    ...
    public async Task Hosted_payload_inspect_classifies_json_and_preserves_content()
    {
        ...
        var inspectedContent = inspection.Content.ShouldNotBeNull();
        inspectedContent.ContentType.ShouldBe(content.ContentType);
        inspectedContent.Encoding.ShouldBe(content.Encoding);
        inspectedContent.Bytes.ToArray().ShouldBe(content.Bytes.ToArray());
    ...
    public async Task Hosted_payload_inspect_exposes_events()
    {
        await WithNodeAsync(async ports =>
        {
            ...
            var eventResult = ports.ReceiveAsync<global::FluxFlow.Data.FlowValue>(Events, Timeout);
            ...
            @event.ToFlowEvent().Name.ShouldBe(PayloadDiagnosticNames.Inspected);
