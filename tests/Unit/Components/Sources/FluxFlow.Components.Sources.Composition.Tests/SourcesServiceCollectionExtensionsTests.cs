...
public sealed class SourcesServiceCollectionExtensionsTests
{
    ...
    public void Design_metadata_provider_describes_source_ports()
    {
        ...
        AssertSourcePorts(metadata[SourcesComponentDefinition.Types.Generated], nameof(FlowValue));
        AssertSourcePorts(metadata[SourcesComponentDefinition.Types.Sequence], nameof(FlowValue));
    }
    ...
    public async Task Hosted_generated_source_binds_items_and_emits_events()
    {
        ...
        var events = (await ports.ObserveAsync<global::FluxFlow.Data.FlowValue>(Events))
            .Observation.ShouldNotBeNull();
    ...
    public async Task Typed_generated_source_authoring_writes_canonical_capacity_and_binds_runtime_options()
    {
        ...
        (await output.Messages.ReceiveAsync().WaitAsync(Timeout)).Value.GetString()
            .ShouldBe("one");
        (await emittedEvent).ToFlowEvent().Attributes["boundedCapacity"].ShouldBe(3d);
    }
    ...
    private static void AssertSourcePorts(ComponentDesignMetadata metadata, string valueType)
    {
        metadata.Ports.Select(port => (
            ...
            (SourcesComponentDefinition.Ports.Events, PortDirection.Output, 1, false, nameof(global::FluxFlow.Data.FlowValue))
        ]);
    }
    ...
    private static async Task<IReadOnlyList<string>> ReceiveEventsThroughAsync(
        ISourceBlock<FlowMessage<global::FluxFlow.Data.FlowValue>> messages,
        string terminalEvent)
    {
        ...
        while (!names.Contains(terminalEvent, StringComparer.Ordinal))
        {
            ...
            names.Add(message.ToFlowEvent().Name);
        }

        return names;
    }

    private static async Task<global::FluxFlow.Data.FlowValue> ReceiveEventAsync(
        ISourceBlock<FlowMessage<global::FluxFlow.Data.FlowValue>> messages,
        string eventName)
    {
        while (true)
        {
            ...
            if (string.Equals(@event.ToFlowEvent().Name, eventName, StringComparison.Ordinal))
                return @event;
        }
    }
