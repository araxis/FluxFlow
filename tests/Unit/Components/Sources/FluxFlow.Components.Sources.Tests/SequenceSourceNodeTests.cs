...
public sealed class SequenceSourceNodeTests
{
    ...
    public async Task Sequence_emits_configured_immutable_values()
    {
        ...
        items.Select(message => Read(message.Value!).Sequence)
            .ShouldBe([1, 2, 3]);
        items.Select(message => Read(message.Value!).Value)
            .ShouldBe([10, 15, 20]);
        items.ShouldAllBe(message =>
            Read(message.Value!).Name == "numbers");
    }
    ...
    public async Task Sequence_uses_configured_clock_for_timing_and_timestamp()
    {
        ...
        items.ShouldAllBe(message =>
            Read(message.Value!).Timestamp >= startInstant);
        items.ShouldAllBe(message =>
            Read(message.Value!).Timestamp <= clock.GetUtcNow());
    }
    ...
    private static SequenceItem Read(FlowValue value)
        => value.ToJsonElement().Deserialize<SequenceItem>(SerializerOptions)!;
}
