...
public sealed class FlowMapperNodeTests
{
    ...
    public async Task Expected_failure_is_an_error_message_and_processing_continues()
    {
        ...
        failedEvent.ToFlowEvent().Name.ShouldBe(FlowMapperNode<string, string>.MapperFailed);
        failedEvent.ToFlowEvent().Level.ShouldBe(FlowEventLevel.Warning);
        var succeededEvent = await events.ReceiveAsync().WaitAsync(Timeout);
        succeededEvent.ToFlowEvent().Name.ShouldBe(FlowMapperNode<string, string>.MapperSucceeded);
    ...
    public async Task Incompatible_expression_output_is_a_normal_error_message()
    {
        ...
        (await node.Input.SendAsync(FlowMessage.Create(
            FlowValue.From(JsonSerializer.SerializeToElement(new { value = "input" }))))).ShouldBeTrue();
    ...
    [Fact]
    public async Task Json_runtime_facade_uses_canonical_ports_and_preserves_lineage()
    {
        var engine = new RecordingExpressionEngine(
            (_, context, _) => context.Variables["input"]);
        await using var node = new JsonMapperNode(
            ...
            engine);
        var results = Sink(node.Output);
        var request = FlowMessage.Create(FlowValue.From(new { value = "input" }));

        (await node.Input.SendAsync(request)).ShouldBeTrue();

        var response = await results.ReceiveAsync().WaitAsync(Timeout);
        response.IsError.ShouldBeFalse();
        response.Value.ToJsonElement().GetProperty("value").GetString().ShouldBe("input");
        response.CorrelationId.ShouldBe(request.CorrelationId);
        response.CausationId.ShouldBe(request.MessageId);
    }
    ...
    public async Task Success_event_uses_configured_clock_and_diagnostic_metadata()
    {
        ...
        @event.ToFlowEvent().Name.ShouldBe(FlowMapperNode<string, string>.MapperSucceeded);
        @event.ToFlowEvent().Attributes["engine"].ShouldBe("test");
        @event.ToFlowEvent().Attributes["expressionId"].ShouldBe("map-v1");
        @event.ToFlowEvent().Attributes["expressionName"].ShouldBe("normalize");
        @event.ToFlowEvent().Attributes["inputType"].ShouldBe(typeof(string).FullName);
        @event.ToFlowEvent().Attributes["outputType"].ShouldBe(typeof(string).FullName);
    }
    ...
    private static async Task ReceiveEventsAsync(BufferBlock<FlowMessage> events, int count)
    {
        for (var index = 0; index < count; index++)
        {
    ...
    private static async Task ReceiveEventAsync(
        BufferBlock<FlowMessage> events,
