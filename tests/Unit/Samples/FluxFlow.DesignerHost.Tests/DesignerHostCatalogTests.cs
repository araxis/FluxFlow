...
public sealed class DesignerHostCatalogTests
{
    ...
    [Fact]
    public void Palette_input_shape_survives_host_json_round_trip_and_undescribed_ports_remain_null()
    {
        var catalog = CreateHostCatalog(new ComponentDesignMetadata
        {
            ...
            [
                new PortDesignMetadata
                {
                    ...
                    InputShape = new FlowValueShape("Request or empty", [JsonValueKind.Object, JsonValueKind.Null], ["target"])
                },
                new PortDesignMetadata
                {
                    Name = new ComponentPortName("Other"),
                    Direction = PortDirection.Input,
                    Order = 1
                },
                new PortDesignMetadata
        ...
        var item = catalog.CreatePaletteItems().ShouldHaveSingleItem();
        var input = item.Inputs.Single(port => port.Name == "Input");

        var shape = input.InputShape.ShouldNotBeNull();
        shape.Description.ShouldBe("Request or empty");
        shape.AcceptedKinds.ShouldBe(["Object", "Null"]);
        shape.RequiredProperties.ShouldBe(["target"]);
        item.Inputs.Single(port => port.Name == "Other").InputShape.ShouldBeNull();
        item.Outputs.ShouldHaveSingleItem().InputShape.ShouldBeNull();

        var restored = JsonSerializer.Deserialize<PortModel>(JsonSerializer.Serialize(input)).ShouldNotBeNull();
        restored.Name.ShouldBe("Input");
        restored.Kind.ShouldBe(PortKind.Input);
        var restoredShape = restored.InputShape.ShouldNotBeNull();
        restoredShape.Description.ShouldBe(shape.Description);
        restoredShape.AcceptedKinds.ShouldBe(shape.AcceptedKinds);
        restoredShape.RequiredProperties.ShouldBe(shape.RequiredProperties);
    }
