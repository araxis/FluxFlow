...
public sealed class ApplicationRuntimeSignalsTests
{
    ...
    public void Signal_payload_json_contracts_are_stable()
    {
        ...
        var error = new DataFlowError(
            ...
            "Condition failed.",
            "link");
        var systemEvent = ApplicationSignalMessage.Create(new FlowEvent
        {
            Timestamp = timestamp,
            ...
            Details = FlowValue.From(new { port = "Main.Source.Output" })
        });
        var diagnostic = ApplicationSignalMessage.Create(new FlowEvent
        {
            Timestamp = timestamp,
            ...
            Measurements = new Dictionary<string, double>(StringComparer.Ordinal) { ["value"] = 125 },
            Details = FlowValue.From(error)
        });

        systemEvent.Headers[FlowEventHeaders.Type].ShouldBe(ApplicationSystemEventNames.LinkConditionFailed);
        systemEvent.ToFlowEvent().Dimensions["category"].ShouldBe("Link");
        systemEvent.ToFlowEvent().Details.Deserialize<JsonElement>()
            .GetProperty("port").GetString().ShouldBe("Main.Source.Output");
        diagnostic.Headers[FlowEventHeaders.Kind].ShouldBe("diagnostic");
        diagnostic.ToFlowEvent().Dimensions["kind"].ShouldBe("Timing");
        diagnostic.ToFlowEvent().Measurements["value"].ShouldBe(125d);
    }
    ...
    public async Task Builder_registers_reserved_system_outputs_with_exact_compiler_metadata()
    {
        ...
        eventPort.PayloadType.ShouldBe(typeof(FlowValue));
    ...
    {
        await using var runtime = new ApplicationPortRuntimeBuilder().Build();
        var slow = new PostponedTarget<FlowMessage>();
        ...
        slow.Accepted.Single().ToFlowEvent().Name.ShouldBe("event-0");
    }
    ...
    public async Task Diagnostics_reject_overflow_promptly_and_preserve_accepted_order()
    {
        ...
        slow.AcceptPostponed();
        slow.Accepted.Single().ToFlowEvent().Name.ShouldBe("diagnostic-0");

        slowLink.Dispose();
        await using var orderedRuntime = new ApplicationPortRuntimeBuilder().Build();
        var ordered = new BufferBlock<FlowMessage>();
        ...
        foreach (var _ in names)
            received.Add((await ordered.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5))).ToFlowEvent().Name);
    ...
    public async Task System_events_flow_through_canonical_links_without_stealing_host_subscriptions()
    {
        await using var runtime = new ApplicationPortRuntimeBuilder()
            .AddFlowValueInput(Input)
            .Build();
        var sink = new BufferBlock<FlowMessage>();
        ...
        var hostReceive = runtime.ReceiveAsync<FlowValue>(
        ...
        (await hostReceive).Message!.ToFlowEvent().Name.ShouldBe("workflow-event");
        (await sink.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5))).ToFlowEvent().Name
            .ShouldBe("workflow-event");
    }
    ...
    public async Task Link_condition_failure_emits_a_trace_correlated_system_event_and_keeps_runtime_active()
    {
        ...
        var receive = runtime.ReceiveAsync<FlowValue>(
            ApplicationAddress.SystemEvents,
            TimeSpan.FromSeconds(5));
        ...
        var projected = systemEvent.ToFlowEvent();
        projected.Name.ShouldBe(ApplicationSystemEventNames.LinkConditionFailed);
        projected.Dimensions["category"].ShouldBe("Link");
        projected.Attributes["error"].ShouldNotBeNull();
    ...
    public async Task Code_predicate_exception_is_route_local_reports_endpoints_and_later_messages_continue()
    {
        ...
        var source = new BufferBlock<FlowMessage<string>>();
        using var sourceAttachment = runtime.AttachOutput(Output, source);
        var eventReceive = runtime.ReceiveAsync<FlowValue>(
            ApplicationAddress.SystemEvents,
            TimeSpan.FromSeconds(5));
        ...
        var projected = systemEvent.ToFlowEvent();
        projected.Name.ShouldBe(ApplicationSystemEventNames.LinkConditionFailed);
        projected.Dimensions["subject"].ShouldBe(Output.Value);
        var projectedError = FlowValue.From(projected.Attributes["error"]).Deserialize<DataFlowError>()!;
        projectedError.Message.ShouldBe(distinctive.Message);
        projectedError.Details!.Value.GetProperty("port").GetString()
            .ShouldBe(Output.Value);
        projectedError.Details.Value.GetProperty("relatedPort").GetString()
    ...
    public async Task Source_fault_emits_component_event_and_only_makes_that_output_unavailable()
    {
        ...
        var systemEvent = (await receive).Message!;
        systemEvent.ToFlowEvent().Name.ShouldBe(ApplicationSystemEventNames.ComponentFaulted);
        systemEvent.ToFlowEvent().Dimensions["category"]
            .ShouldBe("Component");
    ...
    public async Task Status_snapshots_track_port_availability_and_terminal_state()
    {
        ...
        runtime.Status.Ports.Single(port => port.Address == Output)
            .Availability.ShouldBe(ApplicationPortAvailability.Available);
        var lifecycle = runtime.ReceiveAsync<FlowValue>(
            ApplicationAddress.SystemEvents,
            TimeSpan.FromSeconds(5));

        runtime.Complete();
        runtime.Status.State.ShouldBe(ApplicationRuntimeState.Completing);
        (await lifecycle).Message!.ToFlowEvent().Name.ShouldBe(
    ...
    public async Task Port_activity_and_request_timing_are_emitted_as_trace_correlated_diagnostics()
    {
        ...
        var received = new List<FlowMessage>();
        while (received.Select(message => message.ToFlowEvent().Name).Distinct().Count() < 3)
        {
            received.Add(await diagnostics.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5)));
        }

        received.ShouldContain(message =>
            message.ToFlowEvent().Name == ApplicationDiagnosticNames.InputAccepted &&
            message.TraceId == request.TraceId);
        received.ShouldContain(message =>
            message.ToFlowEvent().Name == ApplicationDiagnosticNames.OutputEmitted &&
            message.TraceId == request.TraceId);
        received.ShouldContain(message =>
            message.ToFlowEvent().Name == ApplicationDiagnosticNames.RequestCompleted &&
            message.ToFlowEvent().Measurements["duration.ms"] > 0d &&
            message.TraceId == request.TraceId);
    }
    ...
    {
        await using var runtime = new ApplicationPortRuntimeBuilder()
            .AddFlowValueInput(Input, capacity: 1)
            .Build();
        var target = new PostponedTarget<FlowMessage>();
    ...
    public async Task Signal_publication_honors_cancellation_completion_and_subscriber_failure_isolation()
    {
        ...
        using var failedLink = runtime.SystemEvents.LinkTo(
            new ThrowingTarget<FlowMessage>());
    ...
    public async Task Throwing_subscriber_completion_is_isolated_from_runtime_completion()
    {
        await using var runtime = new ApplicationPortRuntimeBuilder().Build();
        using var failedLink = runtime.SystemEvents.LinkTo(
            new ThrowingCompletionTarget<FlowMessage>(),
            new DataflowLinkOptions { PropagateCompletion = true });
    ...
    private static FlowMessage EventMessage(string name)
    ...
    private static FlowMessage DiagnosticMessage(
        string name,
        string kind = "Log",
        ...
        => ApplicationSignalMessage.Create(new FlowEvent
        {
            ...
            Measurements = measurement is null
                ? new Dictionary<string, double>(StringComparer.Ordinal)
                : new Dictionary<string, double>(StringComparer.Ordinal) { ["value"] = measurement.Value }
        });
    ...
    private static CompiledApplicationLink CompileSystemEventLink()
    {
        ...
        [
            new ComponentDescriptor(
                "sink",
                UnusedFactory,
                inputs: [ComponentPorts.Metadata<FlowValue>("Input")])
        ]);
