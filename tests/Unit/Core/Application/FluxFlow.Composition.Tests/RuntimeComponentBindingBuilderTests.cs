...
public sealed class RuntimeComponentBindingBuilderTests
{
    ...
    public void Typed_factory_registration_declares_metadata_without_running_factory_or_selectors()
    {
        ...
        descriptor.Outputs["Diagnostics"].MessageType.ShouldBe(typeof(global::FluxFlow.Data.FlowValue));
    }

    [Fact]
    public void Runtime_component_binding_builders_expose_only_Has_port_methods()
    {
        AssertCanonicalPortMethods(
            typeof(RuntimeComponentBindingBuilder<>),
            [
                "HasEvents",
                "HasFlowValueInput",
                "HasFlowValueInput",
        ...
        AssertCanonicalPortMethods(
            typeof(RuntimeComponentInstanceBindingBuilder),
            ["HasEvents", "HasFlowValueInput", "HasInput", "HasOutput", "HasSignalInput"]);
    }

    [Fact]
    public async Task Flow_value_bindings_require_physical_canonical_node_ports()
    {
        var services = new ServiceCollection();
        BindingNode? created = null;
        services.AddFluxFlowComponents().Advanced.AddDynamicComponent("test.flow-value", component =>
            component
                .UseFactory(_ => created = new BindingNode())
                .HasFlowValueInput("Input", static node => node.CanonicalInput, FlowValueShape.String("Text"))
                .HasFlowValueOutput("Output", static node => node.CanonicalOutput));
        using var provider = services.BuildServiceProvider();
        var descriptor = provider.GetRequiredService<ComponentDescriptor>();
        var instance = await descriptor.Factory(Context(provider));

        descriptor.Inputs["Input"].MessageType.ShouldBe(typeof(FlowValue));
        descriptor.Outputs["Output"].MessageType.ShouldBe(typeof(FlowValue));
        var input = instance.Inputs["Input"].ShouldBeOfType<FlowValueComponentInputPort>();
        var output = instance.Outputs["Output"].ShouldBeOfType<FlowValueComponentOutputPort>();
        var node = created.ShouldNotBeNull();
        input.Target.ShouldBeSameAs(node.CanonicalInput);
        output.Source.ShouldBeSameAs(node.CanonicalOutput);

        input.Target.Post(FlowMessage.Create(FlowValue.From("payload"))).ShouldBeTrue();
        var delivered = await node.CanonicalInput.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5));
        delivered.IsError.ShouldBeFalse();
        delivered.Value.Deserialize<string>().ShouldBe("payload");

        node.CanonicalOutput.Post(FlowMessage.Create(FlowValue.From(42))).ShouldBeTrue();
        var canonical = await output.Source.ReceiveAsync().WaitAsync(TimeSpan.FromSeconds(5));
        canonical.IsError.ShouldBeFalse();
        canonical.Value.Deserialize<int>().ShouldBe(42);
    ...
    public async Task Sync_factory_infers_ports_and_binds_exact_node_members_once_in_role_order()
    {
        ...
        instance.Outputs["Diagnostics"].ShouldBeOfType<FlowValueComponentOutputPort>();
    ...
    public async Task Multiple_named_event_ports_bind_their_selected_sources_independently()
    {
        ...
        (await diagnosticReceive.WaitAsync(TimeSpan.FromSeconds(5))).ToFlowEvent().Name
            .ShouldBe("diagnostic");
        (await auditReceive.WaitAsync(TimeSpan.FromSeconds(5))).ToFlowEvent().Name
            .ShouldBe("audit");
        descriptor.Outputs.Keys.ShouldBe(["Diagnostics", "Audit"]);
        descriptor.Outputs.Values.All(port => port.MessageType == typeof(global::FluxFlow.Data.FlowValue))
            .ShouldBeTrue();
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void Input_shape_registration_and_preflight_do_not_activate_factories_or_selectors(bool instanceFactory)
    {
        var services = new ServiceCollection();
        var factoryCalls = 0;
        var selectorCalls = 0;
        var shape = FlowValueShape.Object("Request", "target");
        services.AddFluxFlowComponents().Advanced.AddDynamicComponent("test.shape", component =>
        {
            if (instanceFactory)
            {
                component.UseInstanceFactory(_ =>
                {
                    factoryCalls++;
                    throw new InvalidOperationException("Preflight must not activate a component.");
                }).HasFlowValueInput("Input", shape, ComponentPortLinkCardinality.Single);
            }
            else
            {
                component.UseFactory(_ =>
                {
                    factoryCalls++;
                    return new BindingNode();
                }).HasFlowValueInput("Input", node =>
                {
                    selectorCalls++;
                    return node.CanonicalInput;
                }, shape, ComponentPortLinkCardinality.Single);
            }
        });
        using var provider = services.BuildServiceProvider();
        var descriptor = provider.GetRequiredService<ComponentDescriptor>();

        descriptor.Inputs["Input"].InputShape.ShouldBeSameAs(shape);
        descriptor.Inputs["Input"].MessageType.ShouldBe(typeof(FlowValue));
        descriptor.Inputs["Input"].LinkCardinality.ShouldBe(ComponentPortLinkCardinality.Single);
        descriptor.ValidateInputShape("Input", FlowValue.From(new { target = "orders" }))
            .ShouldNotBeNull().IsValid.ShouldBeTrue();
        factoryCalls.ShouldBe(0);
        selectorCalls.ShouldBe(0);
    }

    [Fact]
    public void Equivalent_input_shapes_are_idempotent_and_changed_shapes_conflict()
    {
        var services = new ServiceCollection();
        var builder = services.AddFluxFlowComponents().Advanced;
        Register(FlowValueShape.Object("Request", "target", "body"));
        Register(FlowValueShape.Object("Request", "BODY", "TARGET"));

        services.Count(registration => registration.ServiceType == typeof(ComponentDescriptor)).ShouldBe(1);
        var exception = Should.Throw<InvalidOperationException>(() =>
            Register(FlowValueShape.Object("Request", "target", "body", "method")));
        exception.Message.ShouldContain("test.shape-identity");
        exception.Message.ShouldContain("conflicting descriptor registration");
        services.Count(registration => registration.ServiceType == typeof(ComponentDescriptor)).ShouldBe(1);

        void Register(FlowValueShape shape) => builder.AddDynamicComponent("test.shape-identity", component =>
            component.UseFactory(CreateNode).HasFlowValueInput("Input", SelectCanonicalInput, shape));
    }

    private static ITargetBlock<FlowMessage> SelectCanonicalInput(BindingNode node) => node.CanonicalInput;
    ...
    private static ISourceBlock<FlowMessage> SelectEvents(BindingNode node) => node.Events;
    ...
    private static void AssertCanonicalPortMethods(Type builderType, string[] expectedMethods)
    {
    ...
    private sealed class BindingNode : IFlowNode
    {
        ...
        public BufferBlock<FlowMessage> CanonicalInput { get; } = new();

        public BufferBlock<FlowMessage> CanonicalOutput { get; } = new();

        public BufferBlock<FlowMessage> Events { get; } = new();

        public BufferBlock<FlowMessage> AuditEvents { get; } = new();
        ...
        public void Complete()
        {
            ...
            Output.Complete();
            CanonicalInput.Complete();
            CanonicalOutput.Complete();
            Events.Complete();
        ...
        public void Fault(Exception exception)
        {
            ...
            ((IDataflowBlock)CanonicalInput).Fault(exception);
            ((IDataflowBlock)CanonicalOutput).Fault(exception);
