...
public sealed class FlowContentInspectNodeTests
{
    ...
    public async Task Reuses_flow_content_decode_cache_with_a_host_codec_catalog()
    {
        ...
        await using var node = new FlowContentInspectNode(codecs: catalog);
        var output = Sink(node.Output);
    ...
    public async Task Emits_null_content_as_a_normal_error_result_and_continues()
    {
        ...
        failure.Value.ShouldNotBeNull().ParseError.ShouldContain("requires FlowContent");
