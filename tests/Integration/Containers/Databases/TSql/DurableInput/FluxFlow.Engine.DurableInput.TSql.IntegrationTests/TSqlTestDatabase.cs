...
{
    private readonly List<TSqlDurableInputStore> _stores = [];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var name = "FluxFlowTSqlInputTests_" + Guid.NewGuid().ToString("N");
        ValidateName(name);
    ...
    internal TSqlDurableInputStore CreateStore(TSqlDurableInputStoreOptions? options = null)
    {
        ObjectDisposedException.ThrowIf(Volatile.Read(ref _disposed) != 0, this);
        var store = new TSqlDurableInputStore(options ?? new TSqlDurableInputStoreOptions
        {
            ConnectionString = ConnectionString
        });
        _storeGate.Wait();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText =
            $"ALTER DATABASE [{Name}] SET READ_COMMITTED_SNAPSHOT {(enabled ? "ON" : "OFF")} WITH ROLLBACK IMMEDIATE;";
        command.CommandTimeout = 30;
        await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false);
    }
