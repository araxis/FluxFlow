...
internal sealed class HttpCaptureProbe : IAsyncDisposable
{
    ...
    private HttpCaptureProbe(WebApplication application, Channel<CapturedHttpRequest> requests, Uri captureUrl)
    {
    ...
    internal static async ValueTask<HttpCaptureProbe> StartAsync(CancellationToken cancellationToken = default)
    {
    ...
    internal async ValueTask<CapturedHttpRequest> ReceiveAsync(TimeSpan timeout, CancellationToken cancellationToken = default)
    {
