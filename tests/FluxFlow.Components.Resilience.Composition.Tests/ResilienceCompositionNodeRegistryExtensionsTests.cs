...
public sealed class ResilienceCompositionNodeRegistryExtensionsTests
{
    private static readonly TimeSpan Timeout = TimeSpan.FromSeconds(5);
    private static readonly ApplicationAddress Input =
        ApplicationAddress.WorkflowPort("main", "node", ResilienceCompositionPortNames.Input);
    private static readonly ApplicationAddress Ack =
        ApplicationAddress.WorkflowPort("main", "node", ResilienceCompositionPortNames.Ack);
    private static readonly ApplicationAddress Output =
        ApplicationAddress.WorkflowPort("main", "node", ResilienceCompositionPortNames.Output);
    ...
    public void Designer_metadata_is_valid_and_describes_signal_ports()
    {
        ...
        metadata.Type.Value.ShouldBe(ResilienceCompositionNodeTypes.Retry);
    ...
    public async Task Hosted_retry_accepts_ack_through_signal_port()
    {
        await using var host = await CanonicalApplicationTestHost.StartAsync(
            SingleComponent(
                ResilienceCompositionNodeTypes.Retry,
                Properties(
                    ("strategy", "Fixed"),
                    ("initialDelayMilliseconds", 0),
                    ("maximumAttempts", 2),
                    ("attemptTimeoutMilliseconds", 10_000))),
            registry => registry.RegisterFlowRetry());
    ...
    private static void AssertSignal(CompositionNodeRegistration registration, string name)
    {
        registration.Inputs[name].Kind.ShouldBe(CompositionPortKind.Signal);
