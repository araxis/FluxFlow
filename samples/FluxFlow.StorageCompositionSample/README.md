...
The sample runs four workflows against the same host-owned in-memory store:
...
3. `sample.storage-query-source -> storage.query -> sample.storage-query-result-sink`
4. `sample.storage-delete-source -> storage.delete -> sample.storage-result-sink`
