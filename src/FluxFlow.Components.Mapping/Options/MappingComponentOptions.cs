...
public sealed class MappingComponentOptions
{
    ...
    private readonly Dictionary<Type, IMappingContextFactory> _contextFactories = [];
    ...
    private IMappingContextFactory _defaultContextFactory = new DefaultMappingContextFactory();
    ...
    internal IFlowExpressionEngine ResolveExpressionEngine(string? name)
    {
        if (_expressionEngineResolver is not null)
        {
            ...
            return resolved ?? throw new InvalidOperationException(
                "Mapping expression engine resolver returned null.");
        }

        if (string.IsNullOrWhiteSpace(name))
        {
            return _defaultExpressionEngine ?? throw new InvalidOperationException(
                "Mapping components require an expression engine.");
        }
        ...
        throw new InvalidOperationException(
            $"Mapping expression engine '{name}' is not registered.");
    }
