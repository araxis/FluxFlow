        ...
        bool requireExpression)
    {
        ...
        if (requireExpression && string.IsNullOrWhiteSpace(options.Expression))
        {
            throw new ArgumentException(
                "flow.filter requires configuration value 'expression'.", nameof(options));
        }

        if (string.IsNullOrWhiteSpace(options.InputType))
        {
            throw new ArgumentException(
                "flow.filter option 'inputType' cannot be empty.", nameof(options));
        }
        ...
        if (options.BoundedCapacity <= 0)
        {
            throw new ArgumentOutOfRangeException(
                nameof(options),
                "flow.filter option 'boundedCapacity' must be greater than zero.");
        }
