...
public static class SerializationServiceCollectionExtensions
{
    ...
    internal static void ConfigureJsonParse(ComponentRegistrationBuilder component)
    {
        ...
        component.UseFactory(CreateJsonParseNode)
            .HasFlowValueInput(SerializationComponentDefinition.Ports.Input, static node => node.Input, FlowValueMaterializers.Content.InputShape, "Input", "Messages", 0, "Canonical conversion input.", true)
            .HasFlowValueOutput(SerializationComponentDefinition.Ports.Output, static node => node.Output, "Output", "Results", 1, "Converted value; conversion failures use the message error case.", true)
    ...
    internal static void ConfigureJsonStringify(ComponentRegistrationBuilder component)
    {
        ...
        component.UseFactory(CreateJsonStringifyNode)
            .HasFlowValueInput(SerializationComponentDefinition.Ports.Input, static node => node.Input, FlowValueMaterializers.JsonElement.InputShape, "Input", "Messages", 0, "Canonical conversion input.", true)
            .HasFlowValueOutput(SerializationComponentDefinition.Ports.Output, static node => node.Output, "Output", "Results", 1, "Converted value; conversion failures use the message error case.", true)
            .HasEvents(SerializationComponentDefinition.Ports.Events, static node => node.Events, "Events", "Diagnostics", 2, "Best-effort serialization diagnostics.");
    }
    ...
    internal static void ConfigureTextEncode(ComponentRegistrationBuilder component)
    {
        ...
        component.UseFactory(CreateTextEncodeNode)
            .HasFlowValueInput(SerializationComponentDefinition.Ports.Input, static node => node.Input, FlowValueMaterializers.String.InputShape, "Input", "Messages", 0, "Canonical conversion input.", true)
            .HasFlowValueOutput(SerializationComponentDefinition.Ports.Output, static node => node.Output, "Output", "Results", 1, "Converted value; conversion failures use the message error case.", true)
            .HasEvents(SerializationComponentDefinition.Ports.Events, static node => node.Events, "Events", "Diagnostics", 2, "Best-effort serialization diagnostics.");
    }
