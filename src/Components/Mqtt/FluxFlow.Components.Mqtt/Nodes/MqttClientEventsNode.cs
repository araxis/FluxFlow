...
public sealed class MqttClientEventsNode : IFlowSource
{
    ...
    private readonly MqttSourcePump _pump;
        ...
        int maximumPendingEvents = 128)
    {
        ...
        _pump = new MqttSourcePump(
            maximumPendingEvents,
            RunCoreAsync,
            DisposeCoreAsync);
    }

    public ISourceBlock<FlowMessage> Output => _pump.Output;

    public ISourceBlock<FlowMessage> Events => _pump.Events;
    ...
    private async Task RunCoreAsync(CancellationToken cancellationToken)
    {
        ...
        await foreach (var @event in _subscription.Events
            ...
            .ConfigureAwait(false))
        {
            await _pump.EmitAsync(
                FlowMessage.Create(FlowValue.From<MqttClientEvent>(@event)),
