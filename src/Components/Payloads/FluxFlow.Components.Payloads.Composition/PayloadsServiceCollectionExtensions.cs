...
public static class PayloadsServiceCollectionExtensions
{
    ...
    internal static void ConfigureInspect(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueInput(PayloadsComponentDefinition.Ports.Input, static node => node.Input, FlowValueMaterializers.Content.InputShape, "Input", "Messages", 0, "Canonical content to inspect.", true)
            .HasFlowValueOutput(PayloadsComponentDefinition.Ports.Output, static node => node.Output, "Output", "Results", 1, "Inspection result; content failures use the message error case.", true)
