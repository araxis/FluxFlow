...
public static class ExpectationsServiceCollectionExtensions
{
    ...
    internal static void ConfigureEventExpectation(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueInput(ExpectationsComponentDefinition.Ports.Input, static node => node.Input, new FluxFlow.Components.Projections.Materialization.ProjectionEventMaterializer().InputShape, "Input", "Messages", 0, "Projection event observed by the expectation.", true)
            .HasFlowValueOutput(ExpectationsComponentDefinition.Ports.Output, static node => node.Output, "Output", "Results", 1, "Normal matched, unmet, timeout, completion, or evaluation-failure result.", true)
