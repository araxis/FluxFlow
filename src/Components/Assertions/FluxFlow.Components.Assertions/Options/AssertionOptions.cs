...
/// <summary>
/// Configuration for an assertion node.
/// </summary>
public sealed record AssertionOptions
{
    ...
    internal string EffectiveDescription
        => string.IsNullOrWhiteSpace(Description) ? DefaultDescription : Description.Trim();

    internal string EffectiveFailureMessage
        => string.IsNullOrWhiteSpace(FailureMessage) ? DefaultFailureMessage : FailureMessage.Trim();
}
