...
public static class ProjectionsServiceCollectionExtensions
{
    ...
    internal static void ConfigureProjection(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueInput(ProjectionsComponentDefinition.Ports.Input, static node => node.Input, new FluxFlow.Components.Projections.Materialization.ProjectionEventMaterializer().InputShape, "Input", "Messages", 0, "Projection event to fold into the running snapshot.", true)
            .HasFlowValueOutput(ProjectionsComponentDefinition.Ports.Output, static node => node.Output, "Output", "Results", 1, "Event projection snapshot.", true)
