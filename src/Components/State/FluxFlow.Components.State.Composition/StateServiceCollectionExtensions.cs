...
public static class StateServiceCollectionExtensions
{
    ...
    internal static void ConfigureReducer(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueInput(StateComponentDefinition.Ports.Input, static node => node.Input, new FluxFlow.Components.State.Materialization.StateReducerInputMaterializer().InputShape, "Input", "Messages", 0, "State reducer request.", true)
            .HasFlowValueOutput(StateComponentDefinition.Ports.Output, static node => node.Output, "Output", "Results", 1, "State reducer result.", true)
