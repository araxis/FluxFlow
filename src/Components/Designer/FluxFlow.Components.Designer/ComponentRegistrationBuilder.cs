...
public sealed class ComponentRegistrationBuilder : RuntimeComponentRegistrationBuilder
{
    ...
    internal void DescribeEventShapes(
        string name,
        IEnumerable<EventDesignMetadata>? eventShapes)
        => ReplacePort(name, PortDirection.Output, port => port with
        {
            EventShapes = eventShapes?.ToArray() ?? []
        });
