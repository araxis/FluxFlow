...
public static class ComponentDesignMetadataValidator
{
    public static IReadOnlyList<DesignerMetadataValidationError> Validate(ComponentDesignMetadata metadata)
    {
        ...
        if (metadata.PreferredNodeName is { } preferredNodeName && string.IsNullOrWhiteSpace(preferredNodeName.Value))
        {
            errors.Add(new DesignerMetadataValidationError(
                nameof(ComponentDesignMetadata.PreferredNodeName),
                "Component preferred node name cannot be empty when it is provided."));
        }
        ...
        ICollection<DesignerMetadataValidationError> errors)
    {
        ...
        for (var index = 0; index < ports.Count; index++)
        {
            ...
            if (port.InputShape is not null &&
                (port.Direction != PortDirection.Input ||
                 port.Kind != FluxFlow.Composition.ComponentPortKind.Message ||
                 port.MessageType is not null && port.MessageType != typeof(FluxFlow.Data.FlowValue)))
            {
                errors.Add(new DesignerMetadataValidationError(
                    $"{path}.{nameof(PortDesignMetadata.InputShape)}",
                    "Input shapes can only describe canonical value input ports."));
            }
            ValidateEventShapes(port.EventShapes, $"{path}.{nameof(PortDesignMetadata.EventShapes)}", errors);
            ValidateAttributes(port.Attributes, $"{path}.{nameof(PortDesignMetadata.Attributes)}", errors);
        }
    }

    private static void ValidateEventShapes(
        IReadOnlyList<EventDesignMetadata>? shapes,
        string path,
        ICollection<DesignerMetadataValidationError> errors)
    {
        if (shapes is null)
        {
            errors.Add(new DesignerMetadataValidationError(path, "Event shapes collection is required."));
            return;
        }
        ...
        for (var index = 0; index < shapes.Count; index++)
        {
            ...
            var shapePath = $"{path}[{index}]";
            if (shape is null || string.IsNullOrWhiteSpace(shape.Type))
            {
                errors.Add(new DesignerMetadataValidationError(shapePath, "Event type is required."));
                continue;
            }
            if (!types.Add(shape.Type))
                errors.Add(new DesignerMetadataValidationError(shapePath, $"Event type '{shape.Type}' is duplicated."));
            if (shape.Version < 1)
                errors.Add(new DesignerMetadataValidationError($"{shapePath}.{nameof(EventDesignMetadata.Version)}", "Event version must be positive."));
            if (shape.Fields is null)
            {
                errors.Add(new DesignerMetadataValidationError($"{shapePath}.{nameof(EventDesignMetadata.Fields)}", "Event fields collection is required."));
                continue;
            }
            ...
            {
                if (field is null || string.IsNullOrWhiteSpace(field.Path))
                    errors.Add(new DesignerMetadataValidationError(shapePath, "Event field path is required."));
                else if (!fieldPaths.Add(field.Path))
                    errors.Add(new DesignerMetadataValidationError(shapePath, $"Event field path '{field.Path}' is duplicated."));
            }
        }
    }
        ...
        ICollection<DesignerMetadataValidationError> errors)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(attribute.Key.Value))
            {
