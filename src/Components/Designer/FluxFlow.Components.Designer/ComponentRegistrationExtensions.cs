        ...
        private sealed class PortComparer : IEqualityComparer<PortDesignMetadata>
        {
            ...
            public bool Equals(PortDesignMetadata? left, PortDesignMetadata? right)
                => ReferenceEquals(left, right) ||
                   ...
                   Equals(left.InputShape, right.InputShape) &&
                   EventShapesEqual(left.EventShapes, right.EventShapes) &&
                   DictionariesEqual(left.Attributes, right.Attributes);

            public int GetHashCode(PortDesignMetadata obj) => HashCode.Combine(obj.Name, obj.Direction);

            private static bool EventShapesEqual(
                IReadOnlyList<EventDesignMetadata> left,
                IReadOnlyList<EventDesignMetadata> right)
                => left.Count == right.Count && left.Zip(right).All(static pair =>
                    pair.First.Type == pair.Second.Type &&
                    pair.First.Kind == pair.Second.Kind &&
                    pair.First.Version == pair.Second.Version &&
                    pair.First.Fields.SequenceEqual(pair.Second.Fields));
        }
    }
}
