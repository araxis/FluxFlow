...
public static class HttpServiceCollectionExtensions
{
    ...
    internal static void ConfigureClient(ComponentRegistrationBuilder component)
    {
        ...
        component.WithDisplay(
            ...
            category: "HTTP",
            summary: "Materializes workflow values into HTTP requests and emits canonical response values.",
            iconKey: "send",
            preferredNodeName: "httpClient",
            suggestedEditorWidth: 420);
        component
            .UseFactory(CreateClientNode)
            .HasFlowValueInput(HttpComponentDefinition.Ports.Input, static node => node.Input, new HttpClientRequestMaterializer().InputShape, "Input", "Messages", 0, "HTTP request message.", true)
            .HasFlowValueOutput(HttpComponentDefinition.Ports.Output, static node => node.Output, "Output", "Results", 1, "HTTP response or error result.", true)
