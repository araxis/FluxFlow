...
public static class TimersServiceCollectionExtensions
{
    ...
    internal static void ConfigureInterval(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueOutput(TimersComponentDefinition.Ports.Output, static node => node.Output, "Output", "Messages", 1, "Interval timer tick.", true)
    ...
    internal static void ConfigureSchedule(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueOutput(TimersComponentDefinition.Ports.Output, static node => node.Output, "Output", "Messages", 1, "Scheduled timer tick.", true)
    ...
    internal static void ConfigureDelay(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueInput(TimersComponentDefinition.Ports.Input, static node => node.Input, FlowValueShape.Any("Any canonical workflow value."), "Input", "Messages", 0, "Schema-less JSON value.", true)
            .HasFlowValueOutput(TimersComponentDefinition.Ports.Output, static node => node.Output, "Output", "Messages", 1, "Delayed JSON value; failures use the message error case.", true)
    ...
    internal static void ConfigureThrottle(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueInput(TimersComponentDefinition.Ports.Input, static node => node.Input, FlowValueShape.Any("Any canonical workflow value."), "Input", "Messages", 0, "Schema-less JSON value.", true)
            .HasFlowValueOutput(TimersComponentDefinition.Ports.Output, static node => node.Output, "Output", "Messages", 1, "Rate-limited JSON value; failures use the message error case.", true)
    ...
    internal static void ConfigureDebounce(ComponentRegistrationBuilder component)
    {
        ...
        component
            ...
            .HasFlowValueInput(TimersComponentDefinition.Ports.Input, static node => node.Input, FlowValueShape.Any("Any canonical workflow value."), "Input", "Messages", 0, "Schema-less JSON value.", true)
            .HasFlowValueOutput(TimersComponentDefinition.Ports.Output, static node => node.Output, "Output", "Messages", 1, "Debounced JSON value; failures use the message error case.", true)
    ...
    private static FlowValueTimerDelayNode CreateTimerDelayNode(
        ComponentActivationContext context)
    {
        var settings = context.BindConfiguration<TimerDelaySettings>();
        var clock = context.GetResource<TimeProvider>(
            TimersComponentDefinition.Resources.Clock);
        return new FlowValueTimerDelayNode(settings, clock);
    }

    private static FlowValueTimerThrottleNode CreateTimerThrottleNode(
        ComponentActivationContext context)
    {
        var settings = context.BindConfiguration<TimerThrottleSettings>();
        var clock = context.GetResource<TimeProvider>(
            TimersComponentDefinition.Resources.Clock);
        return new FlowValueTimerThrottleNode(settings, clock);
    }

    private static FlowValueTimerDebounceNode CreateTimerDebounceNode(
        ComponentActivationContext context)
    {
        var settings = context.BindConfiguration<TimerDebounceSettings>();
        var clock = context.GetResource<TimeProvider>(
            TimersComponentDefinition.Resources.Clock);
        return new FlowValueTimerDebounceNode(settings, clock);
    }
}
