...
public sealed class TimerIntervalNode : IFlowSource
{
    ...
    public ISourceBlock<FlowMessage> Output => _source.Output;

    public ISourceBlock<FlowMessage> Events => _source.Events;
...
internal sealed class TimerIntervalSource : FlowSource
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var tick = new TimerIntervalTick(
            timestamp,
            _settings.Name,
            sequence,
            startedAt,
        ...
        await EmitAsync(
            FlowMessage.Create(FlowValue.From(tick)),
