...
/// <summary>
/// Delays canonical workflow values without changing their data contract.
/// </summary>
internal sealed class FlowValueTimerDelayNode : IFlowNode
{
    ...
    private async Task EmitWhenDueAsync(PendingItem pending)
    {
        try
        {
            ...
            await _pipeline.EmitAsync(
                    FlowValueTimerNodeSupport.Success(pending.Message),
                    _pipeline.Stopping)
