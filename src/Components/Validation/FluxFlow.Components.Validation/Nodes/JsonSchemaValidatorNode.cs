...
/// </summary>
public sealed class JsonSchemaValidatorNode : FlowNode
{
    ...
    private FlowMessage Process(FlowMessage message)
    {
        ...
        try
        {
            selectedValue = _selector.Select(input, _nodeContext).Clone();
        }
        ...
        Exception exception)
    {
        var error = new FlowError(
            ...
            category: "Validation",
            isTransient: false,
            details: CreateErrorDetails(input, exception));
        PublishEvent(
            message,
            timestamp,
