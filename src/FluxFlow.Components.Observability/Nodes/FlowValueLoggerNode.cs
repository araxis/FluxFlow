...
/// </summary>
public sealed class FlowValueLoggerNode : IFlowNode
{
    private const string InputType = nameof(FlowValue);

    private readonly FlowValueLoggerOptions _options;
    private readonly FlowLogLevel _level;
    private readonly IReadOnlyDictionary<string, IObservabilityFlowValueSelector> _selectors;
    private readonly ObservabilityNodeContext _nodeContext;
    private readonly TimeProvider _clock;
    private readonly FlowValueObservabilityPipeline<FlowValueLogEntry> _pipeline;
    private long _sequence;

    public FlowValueLoggerNode(
        FlowValueLoggerOptions options,
        IReadOnlyDictionary<string, IObservabilityFlowValueSelector>? attributeSelectors = null,
        TimeProvider? clock = null)
    {
        ...
        {
            NodeType = FlowLoggerNode<FlowValue>.NodeType,
            InputType = typeof(FlowValue),
            Name = _options.EffectiveCategory
        };
        _pipeline = new FlowValueObservabilityPipeline<FlowValueLogEntry>(
    ...
    public ISourceBlock<FlowMessage<FlowResult<FlowValueLogEntry>>> Output
        => _pipeline.Output;
    ...
    private FlowMessage<FlowResult<FlowValueLogEntry>> Process(
        FlowMessage<FlowValue> message)
    {
        ...
        try
        {
            ...
            var entry = new FlowValueLogEntry
            {
                Timestamp = timestamp,
                Level = _level,
            ...
            if (selectorFailures.Count == 0)
            {
                PublishEvent(
                    ...
                    isError: false);
                return message.With(FlowResult<FlowValueLogEntry>.Success(
                    ObservabilityResultKinds.LogEntry,
                    entry,
                    timestamp));
            }
            ...
            PublishEvent(
                ...
                isError: true);
            return message.With(FlowResult<FlowValueLogEntry>.Failure(
                ObservabilityResultKinds.LogEntryPartial,
                error,
                timestamp,
                entry));
    ...
    private FlowMessage<FlowResult<FlowValueLogEntry>> Failure(
        FlowMessage<FlowValue> message,
    ...
    private static IReadOnlyDictionary<string, IObservabilityFlowValueSelector> CopySelectors(
        IReadOnlyDictionary<string, IObservabilityFlowValueSelector>? selectors)
    {
        var copy = new Dictionary<string, IObservabilityFlowValueSelector>(StringComparer.Ordinal);
    ...
    private static FlowValueLoggerOptions ValidateOptions(
        FlowValueLoggerOptions? options)
    {
