...
internal sealed class ApplicationRuntimeAssembler : IAsyncDisposable
{
    ...
    private readonly Queue<FlowMessage> _pendingRevisionEvents = [];
        ...
        IApplicationOutputCaptureResolver? outputCaptures = null)
    {
        ...
        ArgumentNullException.ThrowIfNull(runtimeOptions.Events);
        Events = new Events.ApplicationEventStream(runtimeOptions.Events);
    ...
    internal Events.ApplicationEventStream Events { get; }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var message = ApplicationPortRuntime.CreateRevisionMessage(revisionEvent);
        Events.Publish(message, revisionEvent.RevisionId);
        lock (_eventGate)
        {
            var ports = _ports;
            if (ports is not null)
                return PublishToPortsAsync(ports, message, cancellationToken);

            cancellationToken.ThrowIfCancellationRequested();
            if (_pendingRevisionEvents.Count >= PendingRevisionEventCapacity)
                return ValueTask.FromResult(false);
            _pendingRevisionEvents.Enqueue(message);
    ...
    private static async ValueTask<bool> PublishToPortsAsync(ApplicationPortRuntime ports, FlowMessage message, CancellationToken token)
        => (await ports.PublishSystemEventAsync(message, token).ConfigureAwait(false)).IsAccepted;
        ...
        try
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        if (!await PublishToPortsAsync(generation.Ports, revisionEvent, CancellationToken.None)
                                .ConfigureAwait(false))
                        {
