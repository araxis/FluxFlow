...
public static class FluxFlowServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddExternalFluxFlowInputPort(
        this IServiceCollection services,
        ApplicationAddress address,
        ITargetBlock<FlowMessage> target)
    {
        ArgumentNullException.ThrowIfNull(target);
        ValidateAddress(address, ApplicationAddressKind.WorkflowPort);
        services.AddKeyedSingleton<ITargetBlock<FlowMessage>>(
            address.Value,
            (_, _) => new TargetBlockView<FlowMessage>(target));
        return services;
    }
    ...
    public static IServiceCollection AddExternalFluxFlowOutputPort(
        this IServiceCollection services,
        ApplicationAddress address,
        ISourceBlock<FlowMessage> source)
    {
        ArgumentNullException.ThrowIfNull(source);
        ValidateOutputAddress(address);
        services.AddKeyedSingleton<ISourceBlock<FlowMessage>>(
            address.Value,
            (_, _) => new SourceBlockView<FlowMessage>(source));
        return services;
    }
