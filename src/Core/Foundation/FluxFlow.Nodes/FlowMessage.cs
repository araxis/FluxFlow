...
/// The generic payload keeps component internals and code-first workflows strongly typed.
/// Runtime-authored workflow boundaries use <see cref="FlowValue"/> as <typeparamref name="T"/>
/// and adapt to module-owned types at the component boundary.
/// </summary>
...
public sealed record FlowMessage<T>
{
    ...
    public FlowMessage<TNext> ConvertValue<TNext>(TNext value)
        => new(
            isError: false,
            value,
            error: null,
    ...
    public FlowMessage<TNext> ConvertError<TNext>()
    {
        if (!IsError)
            throw new InvalidOperationException("A value message does not contain an error.");

        return new FlowMessage<TNext>(
            isError: true,
            value: default,
            _error,
            CorrelationId,
    ...
    /// <summary>
    /// Converts this typed helper message to the canonical runtime envelope while
    /// preserving its identity, tracing, timing, and headers.
    /// </summary>
    public FlowMessage ConvertValue(FlowValue value)
    {
    ...
    /// <summary>
    /// Converts this typed error helper message to the canonical runtime envelope
    /// while preserving its identity, tracing, timing, and headers.
    /// </summary>
    public FlowMessage ConvertError()
    {
        if (!IsError)
            throw new InvalidOperationException("A value message does not contain an error.");

        return FlowMessage.RestoreError(
            _error!,
            MessageId,
            TraceId,
            Timestamp,
...
{
    private readonly FlowValue? _value;
    private readonly FlowError? _error;
        ...
        IReadOnlyDictionary<string, string>? headers)
    {
        if (isError && value is not null)
            throw new ArgumentException("An error message cannot contain a value.", nameof(value));
        if (isError && error is null)
            throw new ArgumentException("An error message requires an error.", nameof(error));
        if (!isError && value is null)
            throw new ArgumentException("A value message requires a FlowValue.", nameof(value));
        if (!isError && error is not null)
            throw new ArgumentException("A value message cannot contain an error.", nameof(error));
        if (traceId.IsEmpty)
            throw new ArgumentException("Trace id cannot be empty.", nameof(traceId));
    ...
    public bool TryGetValue([NotNullWhen(true)] out FlowValue? value)
    {
        value = _value;
        return !IsError;
    }

    public bool TryGetError([NotNullWhen(true)] out FlowError? error)
    {
        error = _error;
        return IsError;
    }
    ...
    public TResult Match<TResult>(
        Func<FlowValue, TResult> onValue,
        Func<FlowError, TResult> onError)
    {
        ArgumentNullException.ThrowIfNull(onValue);
        ArgumentNullException.ThrowIfNull(onError);
    ...
    public FlowMessage With(
        FlowValue value,
        IReadOnlyDictionary<string, string>? headers = null,
        MessageId? causationId = null)
    {
        ArgumentNullException.ThrowIfNull(value);
        return CreateDerived(value, error: null, headers, causationId);
    }

    public FlowMessage WithError(
        FlowError error,
        IReadOnlyDictionary<string, string>? headers = null,
        MessageId? causationId = null)
    {
        ArgumentNullException.ThrowIfNull(error);
        return CreateDerived(value: null, error, headers, causationId);
    }
    ...
    public FlowMessage<TNext> ConvertError<TNext>()
    {
        ...
        return FlowMessage<TNext>.Rehydrate(
            isError: true,
            value: default,
            _error,
            CorrelationId,
    ...
    public static FlowMessage Create(
        FlowValue value,
    ...
    {
        ArgumentNullException.ThrowIfNull(value);
        return CreateInitial(value, error: null, correlationId, traceId, headers, causationId);
    }

    public static FlowMessage CreateError(
        FlowError error,
        CorrelationId? correlationId = null,
        TraceId? traceId = null,
        IReadOnlyDictionary<string, string>? headers = null,
    ...
    {
        ArgumentNullException.ThrowIfNull(error);
        return CreateInitial(value: null, error, correlationId, traceId, headers, causationId);
    }
    ...
    public static FlowMessage Restore(
        FlowValue value,
        MessageId messageId,
        TraceId traceId,
        DateTimeOffset timestamp,
    ...
    public static FlowMessage RestoreError(
        FlowError error,
        MessageId messageId,
        TraceId traceId,
        DateTimeOffset timestamp,
    ...
    internal static FlowMessage Rehydrate(
        bool isError,
        FlowValue? value,
        FlowError? error,
        CorrelationId? correlationId,
    ...
    private static FlowMessage CreateInitial(
        FlowValue? value,
        FlowError? error,
        CorrelationId? correlationId,
        TraceId? traceId,
    ...
    private FlowMessage CreateDerived(
        FlowValue? value,
        FlowError? error,
        IReadOnlyDictionary<string, string>? headers,
        MessageId? causationId)
        => new(
            ...
            TraceId,
            global::FluxFlow.Nodes.MessageId.New(),
            causationId is { IsEmpty: false } ? causationId : MessageId,
            DateTimeOffset.UtcNow,
            headers ?? Headers);
